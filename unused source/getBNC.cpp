...
int bncc::processWord(cSource& source, int sourceId, wchar_t* buffer, int tag, int secondTag, int& lastSentenceEnd, int& printLocation, int sentence)
{
	...
	if (!_wcsicmp(sWord.c_str(), L"aged") && tag != TAG_NOT_SET && !wcscmp(tagList[tag].tag, L"PRP"))
	{
		unsigned int I = 0;
		for (; I < sizeof(tagList) / sizeof(*tagList) && wcscmp(tagList[I].tag, L"VVN"); I++);
		tag = I;
	}
	if ((!_wcsicmp(sWord.c_str(), L"t") || !wcsicmp(buffer, L"'t")) && tag != TAG_NOT_SET && !wcscmp(tagList[tag].tag, L"PNP"))
	{
		sWord = L"it";
	...
	if (!_wcsicmp(iWord->first.c_str(), L"ca") || !wcsicmp(iWord->first.c_str(), L"wo") || !wcsicmp(iWord->first.c_str(), L"sha"))
	{
...
// Walk one <s>�</s> body, extract <w TAG> tokens (and ambiguity
