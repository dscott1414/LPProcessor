...
public:
	...
	int answerQuestionInSource(cSource* questionSource, bool parseOnly, bool useParallelQuery, cSyntacticRelationGroup* sri, cSyntacticRelationGroup*& ssri, vector < cTrackDescendantAnswers>& descendantAnswers, bool disableWebSearch);
	...
	bool isQuestionPassive(cSource *questionSource, cSyntacticRelationGroup *sri, cSyntacticRelationGroup * &ssri);
	bool processTransformQuestionPattern(cSource* questionSource, wstring patternType, cSyntacticRelationGroup* sri, cSyntacticRelationGroup*& lssri, cPattern* sourcePattern, cPattern* linkPattern, cSyntacticRelationGroup* linkSyntacticRelationGroup, unordered_map <wstring, wstring>& parseVariables);
	...
	int findMetanamePatterns(cSource* questionSource, cSyntacticRelationGroup* sri);
	int transformQuestion(cSource* questionSource, cSyntacticRelationGroup* sri, cSyntacticRelationGroup*& ssri, vector < cTrackDescendantAnswers>& descendantAnswers, bool parseOnly, bool useParallelQuery, bool disableWebSearch);
	...
	int getWhereQuestionTypeObject(cSource *questionSource, cSyntacticRelationGroup* sri);
	int analyzeQuestionFromSource(cSource *questionSource, wchar_t *derivation, wstring childSourceType, cSource *childSource, cSyntacticRelationGroup * parentSRG, vector < cAS > &answerSRIs, int &maxAnswer, bool eraseIfNoAnswers);
	...
	bool checkIdentical(cSource *questionSource, cSyntacticRelationGroup* sri, cAS &cas1, cAS &cas2);
	void setWhereChildCandidateAnswer(cSource *questionSource, cAS &childCAS, cSyntacticRelationGroup* parentSRG);
	bool enterAnswerAccumulatingIdenticalAnswers(cSource *questionSource, cSyntacticRelationGroup *sri, cAS candidateAnswer, int &maxAnswer, vector < cAS > &answerSRIs);
	int  determineBestAnswers(cSource *questionSource, cSyntacticRelationGroup*  sri,vector < cAS > &answerSRIs,int maxAnswer,vector <cSyntacticRelationGroup> &subQueries,bool useParallelQuery);
	bool isModifiedGeneric(cAS &sri);
	int printAnswers(cSyntacticRelationGroup*  sri, vector < cAS > &answerSRIs);
	int searchWebSearchQueries(cSource *questionSource, wchar_t derivation[1024], cSyntacticRelationGroup* ssri,vector <cSyntacticRelationGroup> &subQueries,
		vector < cAS > &answerSRIs, vector <wstring> &webSearchQueryStrings,
	...
	int findConstrainedAnswers(cSource *questionSource, vector < cAS > &answerSRIs, vector < cTrackDescendantAnswers> &descendantAnswers);
	...
	int	searchTableForAnswer(cSource *questionSource, wchar_t derivation[1024], cSyntacticRelationGroup* sri, unordered_map <int, cWikipediaTableCandidateAnswers * > &wikiTableMap, vector <cSyntacticRelationGroup> &subQueries, vector < cAS > &answerSRIs, int &maxAnswer, bool useParallelQuery);
	...
	bool analyzeRDFTypeBirthDate(cSource* questionSource, cSyntacticRelationGroup* ssri, wstring derivation, vector < cAS >& answerSRIs, int& maxAnswer, wstring birthDate);
	bool analyzeRDFTypeOccupation(cSource* questionSource, cSyntacticRelationGroup* ssri, wstring derivation, vector < cAS >& answerSRIs, int& maxAnswer, wstring occupation);
	bool analyzeRDFTypes(cSource* questionSource, cSyntacticRelationGroup *sqi, cSyntacticRelationGroup* ssri, wstring derivation, vector < cAS >& answerSRIs, int& maxAnswer, unordered_map <int, cWikipediaTableCandidateAnswers* >& wikiTableMap, bool rejectEmptyObjects);
	...
	int analyzeAccumulatedRequests(cSource *questionSource,wchar_t *derivation, cSyntacticRelationGroup *parentSRG, bool parseOnly, vector < cAS > &answerSRIs, int &maxAnswer, vector <cSearchSource> &accumulatedParseRequests);
	int webSearchForQueryParallel(cSource *questionSource, wchar_t *derivation, cSyntacticRelationGroup* parentSRG, bool parseOnly, vector < cAS > &answerSRIs, int &maxAnswer, int webSitesAskedFor, int index, bool googleSearch,
		vector <wstring> &webSearchQueryStrings,int &offset);
	int webSearchForQuerySerial(cSource *questionSource, wchar_t *derivation, cSyntacticRelationGroup* parentSRG, bool parseOnly, vector < cAS > &answerSRIs, int &maxAnswer, int webSitesAskedFor, int index, bool googleSearch,
		vector <wstring> &webSearchQueryStrings,int &offset);
	bool dbSearchMusicBrainzSearchType(cSource *questionSource, wchar_t *derivation, cSyntacticRelationGroup* parentSRG, vector < cAS > &answerSRIs,
		int firstWhere, wstring firstMatchListType, int secondWhere, wstring secondMatchListType, set <wstring> &matchVerbsList);
	bool dbSearchMusicBrainz(cSource *questionSource, wchar_t *derivation, cSyntacticRelationGroup* parentSRG, vector < cAS > &answerSRIs);
	bool dbSearchForQuery(cSource *questionSource, wchar_t *derivation, cSyntacticRelationGroup* parentSRG, vector < cAS > &answerSRIs);
	...
	void detectSubQueries(cSource *questionSource, cSyntacticRelationGroup *sri, vector <cSyntacticRelationGroup> &subQueries);
