...
{
	LFS
		return myquery(&mysql, u"UNLOCK TABLES");
}
...
bool cSource::resetAllSource()
{
	...
	myquery(&mysql, u"update sources set processed=NULL,processing=NULL");
	unlockTables(mysql);
	return true;
}
...
bool cSource::resetSource(int beginSource, int endSource)
{
	...
	lp_snprintf(qt, QUERY_BUFFER_LEN, u"update sources set processed=NULL,processing=NULL where id>=%d and id<%d", beginSource, endSource);
	myquery(&mysql, qt);
	unlockTables(mysql);
	return true;
}
...
void cSource::resetProcessingFlags(void)
{
	...
	myquery(&mysql, u"update sources set processing=NULL");
	unlockTables(mysql);
}
...
bool cSource::signalFinishedProcessingSource(int thisSourceId)
{
	LFS
		if (!myquery(&mysql, u"LOCK TABLES sources WRITE")) return false;
	lpchar_t qt[QUERY_BUFFER_LEN_OVERFLOW];
	lp_snprintf(qt, QUERY_BUFFER_LEN, u"update sources set processing=NULL, processed=true, lastProcessedTime=NOW() where id=%d", thisSourceId);
	myquery(&mysql, qt);
	unlockTables(mysql);
	return true;
}
...
bool getNextUnprocessedSource(MYSQL& mysql, int begin, int end, int sourceType, bool setUsed, int& id, lpwstring& path, lpwstring& encoding, lpwstring& start, int& repeatStart, lpwstring& etext, lpwstring& author, lpwstring& title)
{
	...
	if (!myquery(&mysql, u"LOCK TABLES sources WRITE")) return false;
	lpchar_t qt[QUERY_BUFFER_LEN_OVERFLOW];
	lp_snprintf(qt, QUERY_BUFFER_LEN, u"select id, path, encoding, start, repeatStart, etext, author, title from sources where id>=%d and sourceType=%d and processed IS NULL and processing IS NULL and start!='**SKIP**' and start!='**START NOT FOUND**'", begin, sourceType);
	if (end >= 0)
		lp_snprintf(qt + lp_strlen(qt), QUERY_BUFFER_LEN - lp_strlen(qt), u" and id<%d", end);
	lp_strcpy((qt + lp_strlen(qt)) + lp_strlen(qt + lp_strlen(qt)), u" order by numWords desc limit 1");
	MYSQL_ROW sqlrow = NULL;
	if (myquery(&mysql, qt, result))
	{
		if ((sqlrow = mysql_fetch_row(result)) && setUsed)
		{
			lp_snprintf(qt, QUERY_BUFFER_LEN, u"update sources set processing=true where id=%S", sqlrow[0]);
			myquery(&mysql, qt);
		}
		if (sqlrow != NULL)
		{
...
bool anymoreUnprocessedForUnknown(MYSQL& mysql, int sourceType, int step)
{
	...
	lpchar_t qt[QUERY_BUFFER_LEN_OVERFLOW];
	lp_snprintf(qt, QUERY_BUFFER_LEN, u"select id, etext, path, title from sources where sourceType=%d and processed is not NULL and processing is NULL and start!='**SKIP**' and start!='**START NOT FOUND**' and proc2=%d order by id limit 1", sourceType, step);
	if (myquery(&mysql, qt, result))
	{
...
bool cSource::updateSourceEncoding(int readBufferType, lpwstring sourceEncoding, lpwstring etext)
{
	LFS
		lpwstring tmp, sqlStatement;
	if (sourceEncoding.length() > 55)
		sourceEncoding = sourceEncoding.substr(0, 54);
	sqlStatement = u"update sources set encoding='" + escaped(sourceEncoding) + u"', readBufferFlags=" + itos(readBufferType, tmp) + u" where etext='" + escaped(etext) + u"'";
	return myquery(&mysql, (lpchar_t*)sqlStatement.c_str());
}
...
int getNumSources(MYSQL& mysql, int sourceType, bool left)
{
	...
	MYSQL_ROW sqlrow;
	lpchar_t qt[QUERY_BUFFER_LEN_OVERFLOW];
	if (!myquery(&mysql, u"LOCK TABLES sources READ")) return -1;
	lp_snprintf(qt, QUERY_BUFFER_LEN, u"select COUNT(*) FROM sources where sourceType=%d and start!='**START NOT FOUND**' and start!='**SKIP**'%s", sourceType, (left) ? u" and processed is null" : u"");
...
int initializeDatabaseHandle(MYSQL& mysql, const lpchar_t* where, bool& alreadyConnected)
{
	...
	if (mysql_init(&mysql) == NULL)
		lplog(LOG_FATAL_ERROR, u"Failed to initialize MySQL");
	...
	{
		myquery(&mysql, u"SET NAMES 'utf8mb4' COLLATE 'utf8mb4_bin'");
		mysql_options(&mysql, MYSQL_OPT_RECONNECT, &keep_connect);
		if (mysql_set_character_set(&mysql, "utf8mb4"))
			lplog(LOG_ERROR, u"Error setting default client character set to utf8mb4.  Default now: %S\n", mysql_character_set_name(&mysql));
		return 0;
	}
	return -1;
}
	...
	int numUnmatched, int numOvermatched, int numQuotations, int quotationExceptions, int numTicks, int numPatternMatches)
{
	LFS
		lpchar_t qt[QUERY_BUFFER_LEN_OVERFLOW];
	if (!myquery(&mysql, u"LOCK TABLES sources WRITE")) return;
	lp_snprintf(qt, QUERY_BUFFER_LEN, u"UPDATE sources SET numSentences=%d, matchedSentences=%d, numWords=%d, numUnknown=%d, numUnmatched=%d, "
		u"numOvermatched=%d, numQuotations=%d, quotationExceptions=%d, numTicks=%d, numPatternMatches=%d where id=%d",
...
int cSource::readMultiSourceObjects(tIWMM* wordMap, int numWords)
{
	LFS
		int startTime = clock();
	MYSQL_RES* result = NULL;
	if (!myquery(&mysql, u"select id, objectClass, nickName, ownerObject, firstSpeakerGroup, male, female, neuter, plural FROM objects where common=1", result)) return -1;
	MYSQL_ROW sqlrow;
	...
	mysql_free_result(result);
	if (!myquery(&mysql, u"select objectId, wordId, orderOrType FROM objectWordMap ORDER BY objectId, orderOrType", result)) return -1;
	int previousId = -1;
	while ((sqlrow = mysql_fetch_row(result)) != NULL)
	{
		...
		if (dbi == dbIdToIndex.end())
			lplog(LOG_FATAL_ERROR, u"objectWordMap references common object id %d not present in objects.", dbObjectId);
		...
		if (atoi(sqlrow[1]) >= numWords)
			lplog(LOG_FATAL_ERROR, u"non-existent word reference.");
	...
	if (logDatabaseDetails)
		lplog(u"readMultiSourceObjects took %d seconds.", (clock() - startTime) / CLOCKS_PER_SEC);
	return 0;
}
...
void cSourceWordInfo::transferFormsAndUsage(unsigned int* forms, unsigned int& iCount, int formNum, lpwstring& word)
{
	LFS
		memset(usagePatterns, 0, MAX_USAGE_PATTERNS);
	memset(usageCosts, 0, MAX_USAGE_PATTERNS);
	...
	for (unsigned int I = 0, *J = forms; I < (unsigned)iCount; I++, J += 2)
	{
		...
		if (*J >= (patternFormNumOffset - 1) && *J - (patternFormNumOffset - 1) > MAX_USAGE_PATTERNS - MAX_FORM_USAGE_PATTERNS)
			::lplog(LOG_INFO, u"%s:Incorrect form # - %d.", word.c_str(), *J);
		...
		else
		{
			...
			if (f != fend)
			{
				//::lplog(LOG_INFO,u"Duplicate form %s seen in word %s.",Forms[*J]->name.c_str(),word.c_str());
				continue;
			}
			if (formCount >= MAX_FORM_USAGE_PATTERNS)
			{
				lpwstring formStrings;
				for (int K = 0; K < formCount; K++)
					formStrings += Form(K)->name + u" ";
				::lplog(LOG_DICTIONARY, u"numForms of %s over %d: %s.", word.c_str(), MAX_FORM_USAGE_PATTERNS, formStrings.c_str());
			}
			else
			{
...
bool cSourceWordInfo::updateFromDB(int wordId, unsigned int* forms, unsigned int iCount, int iInflectionFlags, int iFlags, int iTimeFlags, int iMainEntryWordId, int iDerivationRules, int iSourceId, int formNum, lpwstring& word)
{
	LFS
		index = wordId;
...
void cWord::mapWordIdToWordStructure(int wordId, tIWMM iWord)
{
	LFS
		while (idsAllocated <= wordId)
		{
			...
			if (!idToMap)
				lplog(LOG_FATAL_ERROR, u"Out of memory allocating %d bytes to idToMap array.", idsAllocated * sizeof(*idToMap));
...
#ifdef LOG_WORD_FLOW
	lplog(u"Word %s assigned to index %d.", iWord->first.c_str(), wordId);
#endif
...
void cWord::readForms(MYSQL& mysql, lpchar_t* qt)
{
	LFS
		MYSQL_RES* result = NULL;
	MYSQL_ROW sqlrow;
	lp_snprintf(qt, QUERY_BUFFER_LEN, u"SELECT id,name,shortName,inflectionsClass,hasInflections,isTopLevel,properNounSubClass,blockProperNounRecognition from forms");
	if (lastReadfromDBTime > 0)
		lp_snprintf(qt + lp_strlen(qt), QUERY_BUFFER_LEN - lp_strlen(qt), u" where UNIX_TIMESTAMP(ts)>%d", lastReadfromDBTime);
...
#ifdef LOG_WORD_FLOW
			lplog(u"form=%s shortForm=%s inflectionsClass=%s hasInflections=%d properNounSubClass=%d isTopLevel=%d blockProperNounRecognition=%d.",
	...
	if (lastReadfromDBTime > 0 && newDbForms > 0 && Forms.size() > numDbForms)
		lplog(LOG_FATAL_ERROR, u"New forms discovered in db AND new forms already in memory!  Exiting!");
}
...
{
	// Batch B5: POSIX open replaces CreateFile.
	int hFile = ::open("wordFormCache", O_RDONLY);
	if (hFile < 0)
	{
		lplog(LOG_ERROR, u"Failed to open wordFormCache - %d", errno);
		return false;
	}
	// Batch B5: fstat's st_mtime IS a Unix timestamp, so the whole FILETIME dance
	// below it -- assembling a 64-bit count of 100-nanosecond intervals from two
	// 32-bit halves, subtracting the 1601-to-1970 epoch difference, then dividing
	// back down to seconds -- collapses to a single field read.
	struct stat cacheStatus;
	int retCode = (fstat(hFile, &cacheStatus) == 0);
	::close(hFile);
	if (!retCode)
		return false;
	int wordFormCacheLastWriteTime = (int)cacheStatus.st_mtime;
	MYSQL_RES* result = NULL;
	MYSQL_ROW sqlrow;
	// this is insufficient as ISAM table update times are not updated consistently
	//if (!myquery(&mysql, u"SELECT UNIX_TIMESTAMP(UPDATE_TIME) FROM information_schema.tables WHERE TABLE_SCHEMA = 'lp'	AND (TABLE_NAME = 'wordforms' or table_name = 'words')", result)) 
	//	return false;
	bool wordCacheValid = true;
	if (!myquery(&mysql, u"SELECT UNIX_TIMESTAMP(MAX(TS)) FROM words", result))
		return false;
	...
	result = NULL;
	if (!myquery(&mysql, u"SELECT UNIX_TIMESTAMP(MAX(TS)) FROM wordforms", result))
		return false;
	if ((sqlrow = mysql_fetch_row(result)) != NULL)
	{
		int tableTimestamp = atoi(sqlrow[0]);
...
int cWord::readWordFormsFromDB(MYSQL& mysql, int maxWordId, lpchar_t* qt, int* words, int* counts, int& numWordForms, unsigned int*& wordForms, bool printProgress, bool skipWordInitialization, lpwstring specialExtension)
{
	LFS
	...
	if (!testCacheSuccess)
	{
		...
		MYSQL_RES* result = NULL;
		MYSQL_ROW sqlrow;
		int len = lp_snprintf(qt, QUERY_BUFFER_LEN, u"select wf.wordId, wf.formId, wf.count from wordForms wf, words w where w.id=wf.wordId AND w.sourceId IS NULL");
		if (lastReadfromDBTime > 0)
			len += lp_snprintf(qt + len, QUERY_BUFFER_LEN - len, u" AND UNIX_TIMESTAMP(w.ts)>%d", lastReadfromDBTime);
		lp_snprintf(qt + len, QUERY_BUFFER_LEN - len, u" ORDER BY wordId, formId");
		if (!myquery(&mysql, qt, result)) return -1;
		numWordForms = (int)mysql_num_rows(result);
		wordForms = (unsigned int*)tmalloc(numWordForms * sizeof(int) * 2);
		if (printProgress)
			lp_wprintf(u"Reading word forms from database...                        \r");
		while ((sqlrow = mysql_fetch_row(result)) != NULL)
		{
			int wordId = atoi(sqlrow[0]);
			if (wordId<0 || wordId>maxWordId)
				lplog(LOG_FATAL_ERROR, u"wordId < 0 or > %d.", maxWordId);
		...
		if (lastReadfromDBTime == -1)
		{
			...
			if (!myquery(&mysql, u"flush tables")) return -1;  // try to prevent corruption on possible power loss
		}
#endif
	}
	return 2;
...
int getMaxWordId(MYSQL& mysql)
{
	LFS
		MYSQL_RES* result = NULL;
	MYSQL_ROW sqlrow;
	if (!myquery(&mysql, u"select MAX(id) from words", result)) return -1;
...
int cWord::initializeWordsFromDB(MYSQL& mysql, int* words, int* counts, unsigned int*& wordForms, int& numWordsInserted, int& numWordsModified, bool printProgress)
{
	...
	MYSQL_ROW sqlrow;
	if (printProgress)
		lp_wprintf(u"Reading from database: waiting for response from source NULL word query...                        \r");
	// read words
	tIWMM iWord = wNULL;
	int len = lp_snprintf(qt, QUERY_BUFFER_LEN, u"select id, word, inflectionFlags, flags, timeFlags, mainEntryWordId, derivationRules from words where sourceId IS NULL");
	if (lastReadfromDBTime > 0)
		len += lp_snprintf(qt + len, QUERY_BUFFER_LEN - len, u" AND UNIX_TIMESTAMP(ts)>%d", lastReadfromDBTime);
	//lp_snprintf(qt+len,QUERY_BUFFER_LEN-len,u" ORDER BY word");
	if (!myquery(&mysql, qt, result)) return -1;
	int64_t numRows = mysql_num_rows(result);
	WMM.reserve(numRows);
	int where, lastProgressPercent = -1, numWords = 0;
	if (printProgress)
		lp_wprintf(u"Reading from database: reading results from source NULL word query...                        \r");
	...
	while ((sqlrow = mysql_fetch_row(result)) != NULL)
	{
		...
		if (words[wordId] == -1)
		{
			if (!iswdigit(sWord[0]) && sWord[0])
				lplog(LOG_ERROR, u"No forms for %s (wordId=%d) found!", sWord.c_str(), wordId);
			continue;
		}
		...
		if (lastReadfromDBTime >= 0 && (tryWord = WMM.find(sWord)) != WMM.end())
		{
			iWord = tryWord;
#ifdef LOG_WORD_FLOW
			lplog(u"Updated (from sourceId %d) read word %s.", sourceId, sWord.c_str());
#endif
		...
		else
		{
			...
			numWordsInserted++;
#ifdef LOG_WORD_FLOW
			lplog(u"Inserted (from sourceId %d) read word %s.", sourceId, sWord.c_str());
#endif
			mapWordIdToWordStructure(wordId, iWord);
		}
		...
		{
			if (printProgress)
				lp_wprintf(u"PROGRESS: %03d%% words read with %04d seconds elapsed\r", where, clocksec());
		...
		if (iWord->second.query(timeForm) < 0 && iWord->second.query(numberForm) < 0 && iWord->second.query(telenumForm) < 0 &&
			handleExtendedParseWords((lpchar_t*)sWord.c_str())) // if handleExtendedParseWords is true, removed word
			iWord = wNULL;
	}
	mysql_free_result(result);
	...
	for (vector<tIWMM>::iterator wi = wordsToResolve.begin(), wiEnd = wordsToResolve.end(); wi != wiEnd; wi++)
	{
		if (wordStructureGivenWordIdExists((*wi)->second.tmpMainEntryWordId) == wNULL)
		{
			...
			lplog(LOG_FATAL_ERROR, u"mainEntryWordId %d not found for word %s (lastReadfromDBTime=%d)!", (*wi)->second.tmpMainEntryWordId, (*wi)->first.c_str(), lastReadfromDBTime); // mainEntries and any references must always have the same source
	...
	if ((sectionWord = WMM.find(u"|||")) == WMM.end())
	{
		bool added;
		addNewOrModify(NULL, lpwstring(u"|||"), cSourceWordInfo::topLevelSeparator, SECTION_FORM_NUM, 0, 0, u"section", -1, added);
		sectionWord = WMM.find(u"|||");
	}
	if (!myquery(&mysql, u"select UNIX_TIMESTAMP(NOW())", result))
		return -1;
	if ((sqlrow = mysql_fetch_row(result)) == NULL)
		return -1;
	if (sqlrow[0] == NULL)
	{
		if (printProgress)
			lp_wprintf(u"Finished reading from database...                                               \r");
		mysql_free_result(result);
...
int cWord::readWordsFromDB(MYSQL& mysql, bool generateFormStatisticsFlag, bool printProgress, bool skipWordInitialization)
{
	...
	if (printProgress)
		lp_wprintf(u"Started reading from database...                        \r");
	//lplog(u"TIME DB READ START for source %d:%s",sourceId,getTimeStamp());
	lpchar_t qt[QUERY_BUFFER_LEN_OVERFLOW];
	readForms(mysql, qt);
	if (nounForm == -1) nounForm = cForms::gFindForm(u"noun");
	if (verbForm == -1) verbForm = cForms::gFindForm(u"verb");
	...
	if (readWordFormsFromDB(mysql, maxWordId, qt, words, counts, numWordForms, wordForms, printProgress, skipWordInitialization, u"") == 0) return 0;
	if (initializeWordsFromDB(mysql, words, counts, wordForms, numWordsInserted, numWordsModified, printProgress) < 0)
		lplog(LOG_FATAL_ERROR, u"Cannot read words from DB.");
	...
	if (numWordsInserted || numWordsModified)
		lplog(u"%d words inserted into and %d words modified in memory in %d seconds.", numWordsInserted, numWordsModified, (clock() - startTime) / CLOCKS_PER_SEC);
	return 0;
}
...
{
	if (!myquery(&mysql, u"LOCK TABLES openlibraryinternetarchivebooksdump READ"))
		return false;
	MYSQL_RES* result;
	int64_t numResults = 0;
	lpchar_t qt[QUERY_BUFFER_LEN_OVERFLOW];
	lp_snprintf(qt, QUERY_BUFFER_LEN, u"select 1 from openlibraryinternetarchivebooksdump where title = \"%s\"", escaped(proposedTitle).c_str());
	if (myquery(&mysql, qt, result))
	{
		numResults = mysql_num_rows(result);
		mysql_free_result(result);
	...
	if (!myquery(&mysql, u"UNLOCK TABLES"))
		return false;
	lplog(LOG_INFO, u"*** isBookTitle: statement %s resulted in numRows=%d.", qt, numResults);
	return numResults > 0;
}
...
int readWikiNominalizations(MYSQL& mysql, unordered_map <lpwstring, set < lpwstring > >& agentiveNominalizations)
{
	LFS
		MYSQL_RES* result = NULL;
	MYSQL_ROW sqlrow;
	if (!myquery(&mysql, u"select noun,lower(definition) from wiktionarynouns where lower(definition) like '%gent noun of%' or lower(definition) like '%one who %'", result)) return -1;
	//int previousId=-1;
	while ((sqlrow = mysql_fetch_row(result)) != NULL)
	{
		lpwstring noun, verb, definition;
		mTW(sqlrow[0], noun);
		mTW(sqlrow[1], definition);
		size_t pos = definition.find(u"gent noun of");
		if (pos != lpwstring::npos)
		{
			pos += lp_strlen(u"gent noun of ");
			for (; iswalpha(definition[pos]) || definition[pos] == '-'; pos++)
				verb += definition[pos];
		}
		else
		{
			size_t pos2 = definition.find(u"one who ");
			if (pos2 != lpwstring::npos)
			{
				pos2 += lp_strlen(u"one who ");
				for (; iswalpha(definition[pos2]) || definition[pos2] == '-'; pos2++)
					verb += definition[pos2];
				if (verb.empty() || verb == u"is" || verb[verb.length() - 1] != u's' || verb[0] != noun[0])
					verb.clear();
			}
		}
		if (!verb.empty())
		{
			...
			lplog(LOG_WHERE, u"WK %s %s", noun.c_str(), verb.c_str());
		}
	}
	mysql_free_result(result);
	return 0;
...
void maxFieldLengths(const lpwstring key, cOntologyEntry& dbPredicate, int& maxKey, int& maxCompactLabel, int& maxInfoPage, int& maxAbstractDescription, int& maxCommentDescription, int& maxSuperClasses, int& numGT150, int& numGT170, int& numGT190)
{
