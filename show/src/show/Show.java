...
public class Show implements ActionListener, ItemListener {
	...
	static JMenuBar pickSourceMenuBar;
...
//        timelineFrame.setContentPane(timelinePane);
        ...
        timelineFrame.pack();
			    ...
			    public void setValueAt(Object value, int row, int col) {
					...
					class CorrectStrangeBehaviourListener extends ComponentAdapter {
					  public void componentResized(ComponentEvent e){
					    if (wordInfoSelectionTable.getPreferredSize().width <= 	wordInfoScrollPane.getViewport().getExtentSize().width)
			...
			public void initializeDisplay() {
					...
					public void itemStateChanged(ItemEvent e) {
						try {
							if (source.chapters!=null && source.chapters.size()>0)
								gotoPosition(source.sourceToPosition.get(source.chapters.get(pickChapter.getSelectedIndex())+40));
			    ...
			    public void adjustmentValueChanged(AdjustmentEvent ae){
						...
						if (source==null || source.positionToSource==null || source.positionToSource.isEmpty() || lowpos<0 || highpos<0)
							return;
						try{
						...
						for (; lowindex<highindex; lowindex++)
						{
							...
							if (lowws!=null && lowws.flags==SourceMapType.wordType) break;
						}
						for (; highindex>lowindex; highindex--)
						{
							highws = source.positionToSource.get(highindex);
							if (highws!=null && highws.flags==SourceMapType.wordType) break;
						}
        ...
        Boolean[] values = { Boolean.TRUE, Boolean.FALSE, Boolean.FALSE, Boolean.FALSE, Boolean.FALSE, Boolean.FALSE };
        ccb=new ComboCheckBox();
				...
				createSourceMenu();
				//public GridBagConstraints(int gridx = RELATIVE, int gridy= RELATIVE,int gridwidth=1, int gridheight=1,
        //    double weightx=0, double weighty=0,int anchor=CENTER, int fill=NONE,
        //    Insets insets, int ipadx=0, int ipady=0) 
				frame.add(pickPreferences, new GridBagConstraints(0,0,1,1,0,0,GridBagConstraints.CENTER,0,new Insets(0, 0, 0, 0),0,0));
				frame.add(whereField,      new GridBagConstraints(1,0,1,1,1,0,GridBagConstraints.CENTER,GridBagConstraints.HORIZONTAL,new Insets(0, 0, 0, 0),0,0));
				frame.add(pickChapter,     new GridBagConstraints(2,0,1,1,0,0,GridBagConstraints.CENTER,0,new Insets(0, 0, 0, 0),0,0));
				frame.add(reloadButton,    new GridBagConstraints(3,0,1,1,0,0,GridBagConstraints.CENTER,0,new Insets(0, 0, 0, 0),0,0));
				...
				frame.setJMenuBar(pickSourceMenuBar);
				...
				frame.add(infoPanel, new GridBagConstraints(0,1,4,5,0,0,GridBagConstraints.CENTER,GridBagConstraints.HORIZONTAL,new Insets(0, 3, 0, 3),0,0));
				frame.add(mainTextScrollPane, new GridBagConstraints(0,6,4,4,0,1,GridBagConstraints.PAGE_END,GridBagConstraints.BOTH,new Insets(4, 0, 0, 0),0,0));
			...
			{
				DataOutputStream sourceWriter = new DataOutputStream(new BufferedOutputStream(new FileOutputStream("lpGUIState")));
			...
			{
				DataInputStream sourceReader = new DataInputStream(new BufferedInputStream(new FileInputStream("lpGUIState")));
			...
			class DynamicMenu extends JMenu implements ChangeListener, ComponentListener, ActionListener, FocusListener, MenuListener
			{
...
//	    }
	    void createSourceMenu()
			{
	    	pickSourceMenuBar=new JMenuBar();
      ...
      void loadSelectedSource()
		...
		public void run() {
				...
				if (sourcePath==null || !(found=new File(sourcePath + ".wordCacheFile").exists() && new File(sourcePath + ".SourceCache.2").exists()))
				{
					...
					for (Component author: pickSourceMenu.getMenuComponents())
					{
						...
						for (Component book: ((DynamicMenu)author).getMenuComponents())
						{
							...
							if (found=new File(sourcePath + ".wordCacheFile").exists() && new File(sourcePath + ".SourceCache.2").exists())
								break;
						}
						if (found) break;
					}
				...
				MouseListener actionListener = new MouseListener() {
					...
					public void mousePressed(MouseEvent e) {
						...
						case headerType:
							if (ws.index2+1<Source.sections.length)
									try {
										gotoPosition(source.sourceToPosition.get(Source.sections[ws.index2+1].begin+40));
									} catch (BadLocationException e1) {
										// TODO Auto-generated catch block
										e1.printStackTrace();
					...
					public void mouseMoved(MouseEvent e) {
						...
						switch (ws.flags) {
						case wordType:
							wordInfo.setText(ws.index + ": " + source.m[ws.index].getWinnerForms() + source.m[ws.index].word);
							...
							{
								roleInfo.setText(VerbMember.toString(vn.vbNetVerbToClassMap.get(source.m[ws.index].baseVerb)));
						...
						case relType:
							...
							if (r.beforePastHappening) wordInfo.setText("beforePast");
							if (r.futureHappening) wordInfo.setText("future");
							if (r.futureInPastHappening) wordInfo.setText("futureInPast");
							if (r.futureLocation) wordInfo.setText("futureLocation");
							if (r.pastHappening) wordInfo.setText("past");
							if (r.presentHappening) wordInfo.setText("present");
							...
							while (lastMultiple<Source.relations.length && Source.relations[lastMultiple].where==r.where)
								lastMultiple++;
							lastMultiple--;
							relationsInfo.setText(((firstMultiple!=lastMultiple) ? "M ":"")+((r.timeTransition) ? "TT ":"")+r.description);
						...
						case headerType:
							wordInfo.setText(ws.index + ": index2=" + ws.index2 + " index3=" + ws.index3 + " flags=" + ws.flags);
							break;
						case beType:
					      ...
					      return;
							...
							for (int I = fromIndex; I < toIndex; I++) {
								...
								if (ws != null && (lastWS==null || lastWS.index!=ws.index)) {
									...
									String role="";
									wordInfoSelectionTable.setValueAt(ws.index + ":" + source.m[ws.index].getWinnerForms() + source.m[ws.index].word, tableRow, 0);
									wordInfoSelectionTable.setValueAt(role=source.m[ws.index].roleString(), tableRow, 1);
									wordInfoSelectionTable.setValueAt(source.m[ws.index].relations(), tableRow, 2);
									wordInfoSelectionTable.setValueAt(source.objectString(source.m[ws.index].object,false,false), tableRow, 3);
									wordInfoSelectionTable.setValueAt(source.objectString(source.m[ws.index].objectMatches, false, true), tableRow, 4);
									if (source.m[ws.index].object>=0 && source.objects[source.m[ws.index].object].locations!=null)
									System.out.print(source.objectString(source.m[ws.index].object,false,false)+":"+source.objects[source.m[ws.index].object].locations);
									wordInfoSelectionTable.setValueAt(source.m[ws.index].printFlags(), tableRow, 5);
									//System.out.println(ws.index + ": " + source.m[ws.index].word + ":"+source.m[ws.index].relations());
									if (source.m[ws.index].baseVerb.length()>0 && role.length()==0)
									{
										Vector <VerbMember> vms=(source.m[ws.index].baseVerb.length()>0) ? vn.vbNetVerbToClassMap.get(source.m[ws.index].baseVerb) : null;
										if (vms!=null)
										{
											String names="";
