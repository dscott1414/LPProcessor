...
void Source::printLocalFocusedObjects(int where,int forObjectClass)
{ LFS
...
wstring Source::objectString(int object,wstring &logres,bool shortNameFormat,bool objectOwnerRecursionFlag)
{ LFS
  ...
  case UNKNOWN_OBJECT: return logres=L"UNK";
  case OBJECT_UNKNOWN_MALE: return logres=L"UNK_M";
  case OBJECT_UNKNOWN_FEMALE: return logres=L"UNK_F";
  case OBJECT_UNKNOWN_MALE_OR_FEMALE: return logres=L"UNK_M_OR_F";
  case OBJECT_UNKNOWN_NEUTER: return logres=L"UNK_N";
  case OBJECT_UNKNOWN_PLURAL: return logres=L"UNK_P";
  case OBJECT_UNKNOWN_ALL: return logres=L"ALL";
  default:
    return objectString(objects.begin()+object,logres,shortNameFormat,objectOwnerRecursionFlag);
  }
}

wstring Source::objectString(cOM om,wstring &logres,bool shortNameFormat,bool objectOwnerRecursionFlag)
{ LFS
...
wstring Source::objectString(vector <cObject>::iterator object,wstring &logres,bool shortFormat,bool objectOwnerRecursionFlag)
{ LFS
  ...
  if (ow>=0)
  {
	...
	if (object->ownerWhere<-1)
	{
		logres+=wstring(L"{WO:")+wordOrderWords[-2-object->ownerWhere]+L"}";
	}
	if (!shortFormat)
	{
		wchar_t temp[1024];
		...
		if (object->ownerWhere!=-1) logres+=L"[OGEN]";
...
const wchar_t *Source::getOriginalWord(int I, wstring &out, bool concat, bool mostCommon)
{	DLFS
	if (!concat)
		out.clear();
	int len=out.length();
	if (mostCommon)
	{
		...
		out+=m[I].word->first;
  if (m[I].queryForm(PROPER_NOUN_FORM_NUM)>=0 || (m[I].flags&WordMatch::flagFirstLetterCapitalized))
...
void Source::getOriginalWords(int where,vector <wstring> &wsoStrs,bool notFirst)
{ LFS
	wstring oStr;
	oStr[0]=0;
	if (notFirst)
	...
	if (m[where].flags&WordMatch::flagAdjectivalObject)
	{
		...
		if (m[where].objectMatches.empty() && m[where].getObject()>=0)
		{
			...
			if (m[m[where].endObjectPosition-1].flags&WordMatch::flagNounOwner)
			{
				if (logres[logres.size()-1]==L' ')
					logres.erase(logres.size()-1);
				logres+=L"'s";
...
wstring Source::phraseString(int begin,int end,wstring &logres,bool shortFormat,wchar_t *separator)
{ LFS
...
wstring Source::whereString(int where,wstring &logres,bool shortFormat)
{ LFS
	int numWords=0;
	return whereString(where,logres,shortFormat,0,L" ",numWords);
}

wstring Source::whereString(vector <int> &wheres,wstring &logres)
{ LFS
  logres.clear();
...
wstring Source::whereString(set <int> &wheres,wstring &logres)
{ LFS
  logres.clear();
...
wstring Source::whereString(int where,wstring &logres,bool shortFormat,int includeNonMixedCaseDirectlyAttachedPrepositionalPhrases,wchar_t *separator,int &numWords)
{ LFS
	...
	bool isLastUpper=m[lastWord].queryForm(PROPER_NOUN_FORM_NUM)>=0 || (m[lastWord].flags&WordMatch::flagFirstLetterCapitalized) || (m[lastWord].flags&WordMatch::flagAllCaps);
...
wstring Source::objectString(vector <cOM> &oms,wstring &logres,bool shortNameFormat,bool objectOwnerRecursionFlag)
{ LFS
  logres.clear();
...
wstring Source::objectSortedString(vector <cOM> &objs,wstring &logres)
{ LFS
...
wstring Source::objectString(vector <int> &objectVector,wstring &logres)
{ LFS
  logres.clear();
...
wstring Source::objectString(cSpeakerGroup::cGroup &group,wstring &logres)
{ LFS
...
wstring Source::objectSortedString(vector <int> &objs,wstring &logres)
{ LFS
...
wstring Source::wordString(vector <tIWMM> &words,wstring &logres)
{ LFS
  logres.clear();
...
wstring Source::objectString(set <int> &objs,wstring &logres,bool shortNameFormat)
{ LFS
  logres.clear();
...
void Source::printSpeakerQueue(int where)
{ LFS
...
bool Source::quotedString(unsigned int beginQuote,unsigned int endQuote,bool &noTextBeforeOrAfter,bool &noSpeakerAfterward)
{ LFS
  if (noSpeakerAfterward=((m[endQuote].flags&WordMatch::flagInsertedQuote)!=0)) return false;
  bool EOSEmbeddedInside=m[endQuote-1].word->first==L"." || m[endQuote-1].word->first==L"?" || m[endQuote-1].word->first==L"!";
	...
	{
		for (vector <WordMatch>::iterator im=m.begin()+beginQuote+1,imEnd=m.begin()+endQuote-1; im!=imEnd; im++)
...
wchar_t *wordOrderWords[]={L"other",L"another",L"second",L"first",L"third",L"former",L"latter",L"that",L"this",L"two",L"three",L"one",L"four",L"five",L"six",L"seven",L"eight",NULL};

// a word indicating it is an object which is in the same speakerGroup as its owner
bool Source::isMetaGroupWord(int where)
{ LFS
...
bool Source::isNonTransferrableAdjective(tIWMM word)
{ LFS
...
bool Source::isExternalBodyPart(int where,bool &singular,bool pluralAllowed)
{ LFS
	singular=false;
...
bool Source::isPsychologicalFeature(int where)
{ LFS
	tIWMM w;
	...
	if (!pfInit)
	{
		...
		for (set <wstring>::iterator pi=psychological.begin(),piEnd=psychological.end(); pi!=piEnd; pi++)
		{
			...
			{
				if (w!=Words.end() && w->second.query(nounForm)<0) continue;
				if (!(w->second.flags&tFI::physicalObjectByWN)) 
					w->second.flags|=tFI::notPhysicalObjectByWN;
				w=w->second.mainEntry;
			}
			if (w!=Words.end() && w->second.query(nounForm)<0) continue;
			if (w!=Words.end() && !(w->second.flags&tFI::physicalObjectByWN)) 
				w->second.flags|=tFI::notPhysicalObjectByWN;
		}
	...
	w=m[where].word;
	if (w->second.mainEntry!=wNULL) return (w->second.mainEntry->second.flags&tFI::notPhysicalObjectByWN)!=0;
	return (w->second.flags&tFI::notPhysicalObjectByWN)!=0;
}
*/

bool Source::isFace(int where)
{ LFS
...
bool Source::isFacialExpression(int where)
{ LFS
	if (where<0) return false;
	wstring word=m[where].word->first;
...
bool Source::isInternalBodyPart(int where)
{ LFS
	wstring word=m[where].word->first;
...
// he was conscious
bool Source::isInternalDescription(int where)
{ LFS
	wstring word=m[where].word->first;
...
bool Source::isPossible(int where)
{ LFS
...
bool Source::isGroupJoiner(tIWMM word)
{ LFS
...
bool Source::isDelayedReceiver(tIWMM word)
{ LFS
...
bool Source::isKindOf(int where)
{ LFS
...
bool Source::isVision(int where)
{ LFS
...
bool Source::overlaps(vector <cObject>::iterator object,vector <cObject>::iterator matchingObject)
{ LFS
...
bool Source::getHighestEncounters(int &highestDefinitelyIdentifiedEncounters,int &highestIdentifiedEncounters,int &highestEncounters)
{ LFS
...
void Source::getOwnerSex(int ownerWhere,bool &matchMale,bool &matchFemale,bool &matchNeuter, bool &matchPlural)
{ LFS
  ...
  case OBJECT_UNKNOWN_MALE: matchMale=true; return;
  case OBJECT_UNKNOWN_FEMALE: matchFemale=true; return;
  case OBJECT_UNKNOWN_MALE_OR_FEMALE: matchMale=matchFemale=true; return;
  case OBJECT_UNKNOWN_NEUTER: matchNeuter=true; return;
  case OBJECT_UNKNOWN_PLURAL: matchPlural=true; return;
  default:
    if (ownerObject>=0)
    {
...
wstring Source::getClass(int objectClass)
{ LFS
  switch (objectClass)
  {
...
void Source::adjustForPhysicalPresence()
{ LFS
...
// in secondary quotes, inPrimaryQuote=false
void Source::adjustSaliencesByGenderNumberAndOccurrenceAgeAdjust(int where,int object,bool inPrimaryQuote,bool inSecondaryQuote,bool forSpeakerIdentification,int &lastGenderedAge,vector <int> &disallowedReferences,bool disallowOnlyNeuterMatches,bool isPhysicallyPresent,bool physicallyEvaluated)
{ LFS
  for (int tries=0; tries<3; tries++)
  {
    ...
    for (; lsi!=lsEnd; lsi++)
    {
      ...
      lsi->res.clear();
		...
		if (object>=0 && objects[object].objectClass!=BODY_OBJECT_CLASS && objects[object].ownerWhere>=0 && 
			  m[objects[object].ownerWhere].objectMatches.size()==1 && (lsi=in(m[objects[object].ownerWhere].objectMatches[0].object))!=localObjects.end())
		{
			wstring tmpstr;
			if (debugTrace.traceSpeakerResolution)
    ...
    adjustSaliencesByGenderNumberAndOccurrence(where,object,inPrimaryQuote,inSecondaryQuote,forSpeakerIdentification,tmpLastGenderedAge,isPhysicallyPresent);
		for (lsi=localObjects.begin(); lsi!=lsEnd; lsi++)
			if (find(disallowedReferences.begin(),disallowedReferences.end(),lsi->om.object)!=disallowedReferences.end() ||
				  (objects[lsi->om.object].objectClass==BODY_OBJECT_CLASS && objects[lsi->om.object].ownerWhere>=0 && 
					 find(disallowedReferences.begin(),disallowedReferences.end(),m[objects[lsi->om.object].ownerWhere].getObject())!=disallowedReferences.end()))
			{
				wstring tmpstr;
				lsi->om.salienceFactor-=DISALLOW_SALIENCE;
    ...
    if (m[where].getObject()==OBJECT_UNKNOWN_NEUTER ||
...
void Source::printNyms(vector <tIWMM> &nyms1, map <tIWMM,vector <tIWMM>,tFI::cRMap::wordMapCompare > &wnMap,wchar_t *type,wchar_t *subtype,wchar_t *subsubtype)
{ LFS
	wstring nyms;
	for (vector <tIWMM>::iterator ni=nyms1.begin(),niEnd=nyms1.end(); ni!=niEnd; ni++)
	{
		if ((*ni)->second.flags&tFI::genericGenderIgnoreMatch) continue;
		nyms+=(*ni)->first+L" (";
...
void Source::printNyms(vector <tIWMM> &nyms1, map <tIWMM,vector <tIWMM>,tFI::cRMap::wordMapCompare > &wnMap1,
...
bool Source::setNyms(vector <tIWMM> &nyms1, map <tIWMM,vector <tIWMM>,tFI::cRMap::wordMapCompare > &wnMap)
{ LFS
	for (vector <tIWMM>::iterator ni=nyms1.begin(),niEnd=nyms1.end(); ni!=niEnd; ni++)
	{
		if (!((*ni)->second.flags&tFI::alreadyTaken))
		{
			(*ni)->second.flags|=tFI::intersectionGroup;
			for (vector <tIWMM>::iterator cni=wnMap[*ni].begin(),cniEnd=wnMap[*ni].end(); cni!=cniEnd; cni++)
				if (!((*cni)->second.flags&tFI::alreadyTaken))
					(*cni)->second.flags|=tFI::intersectionGroup;
		}
	}
	return !nyms1.empty();
}

void Source::clearNyms(vector <tIWMM> &nyms1, map <tIWMM,vector <tIWMM>,tFI::cRMap::wordMapCompare > &wnMap)
{ LFS
	for (vector <tIWMM>::iterator ni=nyms1.begin(),niEnd=nyms1.end(); ni!=niEnd; ni++)
	{
		(*ni)->second.flags&=~tFI::intersectionGroup;
...
int Source::nymMapMatch(vector <tIWMM> &nyms1, map <tIWMM,vector <tIWMM>,tFI::cRMap::wordMapCompare > &wnMap1,
...
{ LFS
	...
	{
		if ((*ni2)->second.flags&tFI::genericGenderIgnoreMatch) continue;
		if (!mapOnly && ((*ni2)->second.flags&(tFI::intersectionGroup|tFI::alreadyTaken))==tFI::intersectionGroup)
		{
			if (getFromMatch)
			...
			(*ni2)->second.flags|=tFI::alreadyTaken;
			markedTaken.push_back(*ni2);
		}
		if (!((*ni2)->second.flags&tFI::alreadyTaken))
		{
			for (vector <tIWMM>::iterator cni2=wnMap2[*ni2].begin(),cni2End=wnMap2[*ni2].end(); cni2!=cni2End; cni2++)
				if (((*cni2)->second.flags&(tFI::intersectionGroup|tFI::alreadyTaken|tFI::genericGenderIgnoreMatch))==tFI::intersectionGroup)
				{
					if (getFromMatch)
						for (vector <tIWMM>::iterator ni1=nyms1.begin(),ni1End=nyms1.end(); ni1!=ni1End; ni1++)
						{
					...
					if (!mapOnly)
					{
						(*cni2)->second.flags|=tFI::alreadyTaken;
						markedTaken.push_back(*cni2);
					}
				}
				else if (traceNymMatch)
					lplog(LOG_RESOLUTION,L"(*cni2)->first=%s ((*cni2)->second.flags&(tFI::intersectionGroup|tFI::alreadyTaken|tFI::genericGenderIgnoreMatch))=%s %s %s",
					      (*cni2)->first.c_str(),
								((*cni2)->second.flags&(tFI::intersectionGroup)) ? L"true" : L"false",
								((*cni2)->second.flags&(tFI::alreadyTaken)) ? L"true" : L"false",
								((*cni2)->second.flags&(tFI::genericGenderIgnoreMatch)) ? L"true" : L"false");
		}
		else if (traceNymMatch)
			lplog(LOG_RESOLUTION,L"(*ni2)->first=%s ((*ni2)->second.flags&tFI::alreadyTaken)=%s",(*ni2)->first.c_str(),((*ni2)->second.flags&tFI::alreadyTaken) ? L"true" : L"false");
	...
	for (vector <tIWMM>::iterator mi=markedTaken.begin(),miEnd=markedTaken.end(); mi!=miEnd; mi++)
		(*mi)->second.flags&=~tFI::alreadyTaken;
...
void Source::adjustSaliencesByGenderNumberAndOccurrence(int where,int object,bool inPrimaryQuote,bool inSecondaryQuote,bool forSpeakerIdentification,int &lastGenderedAge,bool isPhysicallyPresent)
{ LFS
  wstring tmpstr;
  ...
  if (object>=0)
  {
    ...
    matchNeuter=neuter=o->neuter;
		if (objects[object].objectClass==BODY_OBJECT_CLASS) 
			getOwnerSex(o->ownerWhere,matchMale,matchFemale,matchNeuter,matchPlural);
  }
  else
  {
    ...
    if (m[where].queryWinnerForm(PROPER_NOUN_FORM_NUM)>=0 || (m[where].flags&WordMatch::flagFirstLetterCapitalized)!=0 ||
  ...
  for (; lsi!=lsEnd; lsi++)
  {
    ...
    if (lso->objectClass==BODY_OBJECT_CLASS && lso->ownerWhere>=0 && !neuter)
		{
			vector <cLocalFocus>::iterator lsbo=localObjects.end();
			if (m[lso->ownerWhere].objectMatches.size()==1) lsbo=in(m[lso->ownerWhere].objectMatches[0].object);
			if (m[lso->ownerWhere].objectMatches.empty()) lsbo=in(m[lso->ownerWhere].getObject());
    ...
    if (lso->objectClass==BODY_OBJECT_CLASS)
      getOwnerSex(lso->ownerWhere,localMale,localFemale,localNeuter,localPlural);
		...
		if ((object>=0 && o->ownerWhere<-1) || (lsi->lastWhere>=0 && m[lsi->lastWhere].word->first==L"all")) ignoreMPlural=ignorePlural=true;
    ...
    if (debugTrace.traceObjectResolution || debugTrace.traceSpeakerResolution)
    {
      ...
      else if (lsi->lastWhere>=0 && !ignoreMPlural)
      {
        ...
        if (lsi->previousWhere>=0)
        {
          ...
          (!(m[lastOpeningPrimaryQuote].flags&WordMatch::flagQuoteContainsSpeaker) || !in(lsi->om.object,m[lastOpeningPrimaryQuote].getRelObject())))
...
bool Source::disallowReference(int object,vector <int> &disallowedReferences)
{ LFS
	if (object>=0)
	{
...
void Source::checkForPreviousPP(int where,vector <int> &disallowedReferences)
{ LFS
...
int Source::getRoleSalience(unsigned __int64 role)
{ LFS
...
void Source::localRoleBoost(vector <cLocalFocus>::iterator lsi,int I,unsigned __int64 objectRole,int age)
{ LFS
...
void Source::adjustSaliencesByParallelRoleAndPlurality(int where,bool inPrimaryQuote,bool forSpeakerIdentification,int lastGenderedAge)
{ LFS
  vector <cLocalFocus>::iterator lsi=localObjects.begin(),lsEnd=localObjects.end();
  for (; lsi!=lsEnd; lsi++)
		if (lsi->includeInSalience(objectToBeMatchedInQuote,quoteIndependentAge) && lsi->om.salienceFactor>=0)
  ...
  for (int I=where-1; I>=0 && age<MAX_PARALLEL_ROLE_AGE+lastGenderedAge; I--)
  {
    ...
    if (age>=MAX_PARALLEL_ROLE_AGE+lastGenderedAge) break;
		...
		bool plural=(o==OBJECT_UNKNOWN_PLURAL || (o>=0 && objects[o].plural)),allowEitherPlural=(m[I].objectRole&MPLURAL_ROLE)!=0 || (o>=0 && objects[o].objectClass==BODY_OBJECT_CLASS && objects[o].plural);
    ...
    if ((o>=0 && (lsi=objects[o].lsiOffset)!=cNULL) || 
		...
		if (quoteMatch && pluralMatch && 
			  (o<0 || objects[o].objectClass!=BODY_OBJECT_CLASS || objects[o].ownerWhere<0))
...
bool Source::potentiallyMergable(int where,vector <cObject>::iterator o1,vector <cObject>::iterator o2,bool allowBothToBeSpeakers,bool checkUnmergableSpeaker)
{ LFS
  if (o1==o2 || matchAliases(where,(int)(o1-objects.begin()),(int)(o2-objects.begin())))
    return true;
	bool switched=false;
  if (switched=o1->objectClass==BODY_OBJECT_CLASS && o1->ownerWhere>=0 && m[o1->ownerWhere].getObject()>=0) o1=objects.begin()+m[o1->ownerWhere].getObject();
  if (o2->objectClass==BODY_OBJECT_CLASS && o2->ownerWhere>=0 && m[o2->ownerWhere].getObject()>=0) 
	{
		switched=true;
		o2=objects.begin()+m[o2->ownerWhere].getObject();
	}
...
vector <cOM>::iterator Source::in(int o,vector <cOM> &objs)
{ LFS
...
bool Source::in(int o,int where)
{ LFS
	if (where<0) return false;
...
vector <cLocalFocus>::iterator Source::in(int o)
{ LFS
...
bool Source::replaceObject(int replacementObject,int objectToBeReplaced,vector <cOM> &objectList,wchar_t *fromWhat)
{ LFS
...
bool Source::replaceObject(int replacementObject,int objectToBeReplaced,vector <cOM> &objectList,vector <cObject::cLocation>::iterator ol)
{ LFS
  vector <cOM>::iterator omi,omEnd=objectList.end(),keep;
  if ((omi=in(objectToBeReplaced,objectList))==omEnd) return false;
  keep=omi;
...
bool Source::replaceObject(int replacementObject,int objectToBeReplaced,vector <cOM> &objectList,int ol)
{ LFS
  vector <cOM>::iterator omi,omEnd=objectList.end(),keep;
  if ((omi=in(objectToBeReplaced,objectList))==omEnd) return false;
  keep=omi;
...
void Source::replaceObject(int replacementObject, int objectToBeReplaced,wchar_t *fromWhat)
{ LFS
...
bool Source::replaceObject(int where,int replacementObject, int objectToBeReplaced,set <int> &objectSet, wchar_t *description, int begin, int end,wchar_t *fromWhat)
{ LFS
...
bool Source::replaceObject(int where,int replacementObject, int objectToBeReplaced,vector <int> &objectList, wchar_t *description, int begin, int end,wchar_t *fromWhat)
{ LFS
	if (replacementObject==objectToBeReplaced)
		return false;
...
void Source::moveNyms(int where,int toObject,int fromObject,wchar_t *fromWhere)
{ LFS
	wstring tmpstr,tmpstr2;
	...
	if (!(m[objects[toObject].originalLocation].word->second.flags&tFI::genericGenderIgnoreMatch) && 
...
bool Source::replaceObjectInSpeakerGroup(int where,int replacementObject,int objectToBeReplaced,int sg,wchar_t *fromWhat)
{ LFS
...
void Source::replaceObjectWithObject(int where,vector <cObject>::iterator &object,int replacementObject,wchar_t *fromWhat)
{ LFS
  wstring tmpstr,tmpstr2;
  ...
  if (objects[replacementObject].firstSpeakerGroup<0) objects[replacementObject].firstSpeakerGroup=object->firstSpeakerGroup;
...
bool Source::recentExit(vector <cLocalFocus>::iterator lsi)
{ LFS
...
void Source::testLocalFocus(int where,vector <cLocalFocus>::iterator lsi)
{ LFS
...
// in secondary quotes, inPrimaryQuote=false
bool Source::pushObjectIntoLocalFocus(int where, int matchingObject, bool identifiedAsSpeaker, bool notSpeaker, bool inPrimaryQuote, bool inSecondaryQuote, wchar_t *fromWhere, vector <cLocalFocus>::iterator &lsi)
{ LFS
	...
	if (lsi==localObjects.end())
  {
    ...
    lsi=localObjects.begin()+localObjects.size()-1;
		...
		if (!notSpeaker && 
			  ((m[where].flags&WordMatch::flagUnresolvableObjectResolvedThroughSpeakerGroup) || 
  ...
  else
	{
		...
		testLocalFocus(where,lsi);
		//if (m[where].getObject()>=0 && objects[m[where].getObject()].objectClass==BODY_OBJECT_CLASS && (m[where].word->second.flags&tFI::physicalObjectByWN)==0)
		//{
		//	currentlyPhysicallyPresent=false;
		//}
...
vector <cLocalFocus>::iterator Source::substituteAlias(int where,vector <cLocalFocus>::iterator lsi)
{ LFS
...
void Source::pushLocalObjectOntoMatches(int where,vector <cLocalFocus>::iterator lsi,wchar_t *reason)
{ LFS
  wstring tmpstr;
  ...
  else
  {
    if (in(lsi->om.object,m[where].objectMatches)!=m[where].objectMatches.end()) return;
		...
		// substitute a preferred class alias ('Boris' for 'a Russian')
...
void Source::narrowGender(int where,int toObject)
{ LFS
  ...
  if (objects[toObject].objectClass==BODY_OBJECT_CLASS && objects[toObject].ownerWhere>=0)
  {
    toObject=m[objects[toObject].ownerWhere].getObject();
    if (toObject<0 || !(objects[toObject].male && objects[toObject].female))
      return;
  }
	wstring tmpstr;
  if (object<UNKNOWN_OBJECT)
  {
...
void Source::eliminateBodyObjectRedundancy(int where,vector <cOM> &objectMatches)
{ LFS
  for (vector <cOM>::iterator omi=objectMatches.begin(); omi!=objectMatches.end(); )
  {
    bool allIn,oneIn;
		int ownerWhere=objects[omi->object].ownerWhere;
...
bool Source::evaluateCompoundObjectAsGroup(int where,bool &physicallyEvaluated)
{ LFS
...
#define HIGHEST 8
void Source::matchAdditionalObjectsIfPlural(int where,bool isPlural,bool atLeastOneReference,
...
{ LFS
	...
	for (int I=0; I<(signed)m[where].objectMatches.size(); I++)
	{
		...
		if ((rm=w->second.relationMaps[relationType]) && ((tr=rm->r.find(verb))!=rm->r.end()))
		{
			wordsWithFrequency++;
...
// use partial_sort?
// in secondary quotes, inPrimaryQuote=false
bool Source::chooseBest(int where,bool definitelySpeaker,bool inPrimaryQuote,bool inSecondaryQuote,bool resolveForSpeaker,bool mixedPlurality)
{ LFS
  // get three highest choices > 0 in ls
	...
	bool genericGender=(m[where].word->second.flags&tFI::genericGenderIgnoreMatch) && (m[where].endObjectPosition-m[where].beginObjectPosition)==2 && m[m[where].beginObjectPosition].word->first==L"the";
	...
	for (vector <cLocalFocus>::iterator lsi=localObjects.begin(), lsiEnd=localObjects.end(); lsi!=lsiEnd; lsi++)
  {
    ...
    int lso=lsi->om.object,lsow=objects[lso].ownerWhere;
    ...
    if (lsow>=0 && (m[lsow].getObject()==object || in(object,m[lsow].objectMatches)!=m[lsow].objectMatches.end()))
      continue;
		...
		if (ca && !lsi->physicallyPresent && physicallyPresent && (sf<0 || (lsi->notSpeaker && lsi->lastWhere>=0 && (m[lsi->lastWhere].flags&WordMatch::flagAdjectivalObject))) && 
		...
		int ms=(ca<=1 || (object>=0 && objects[object].ownerWhere<-3 && !cLocalFocus::salienceIndependent(quoteIndependentAge)) || (ca==2 && genericGender) ||
  ...
  // if choosing between two speakers
      ...
      highest[0]->om.salienceFactor>=SALIENCE_THRESHOLD && highest[1]->om.salienceFactor>=SALIENCE_THRESHOLD)
	{
		...
		// if choosing between two speakers, and quote has embeddedStory and the quote before also has embedded story, then prefer the previous speaker
				...
				(m[lastOpeningPrimaryQuote].flags&WordMatch::flagEmbeddedStoryResolveSpeakers) && previousPrimaryQuote>=0 && (m[previousPrimaryQuote].flags&WordMatch::flagEmbeddedStoryResolveSpeakers) &&
		...
		*/
		...
		if (previousPrimaryQuote>=0 && (m[where].objectRole&HAIL_ROLE))
		{
			...
			if (where==78019)
			{
				...
				lplog(LOG_RESOLUTION,L"trackBack=%d previousPrimaryQuote=%d %s %d!=%d  %d==%d  %s %s %s",trackBack,previousPrimaryQuote,
							...
							m[previousPrimaryQuote].quoteForwardLink,lastOpeningPrimaryQuote,
							m[trackBack].nextQuote,lastOpeningPrimaryQuote,
							(m[previousPrimaryQuote].flags&WordMatch::flagLastContinuousQuote) ? L"true":L"false",
							roleString(where,tmpstr).c_str(),
							objectString(m[previousPrimaryQuote].objectMatches,tmpstr2,true).c_str());
			}
			*/
			if (((m[trackBack].flags&WordMatch::flagDefiniteResolveSpeakers) || 
				...
				m[previousPrimaryQuote].quoteForwardLink!=lastOpeningPrimaryQuote && // just in case this is introduced in the same paragraph but in a second quote
				m[trackBack].nextQuote==lastOpeningPrimaryQuote && !(m[previousPrimaryQuote].flags&WordMatch::flagLastContinuousQuote) && // quote immediately before - no intervening paragraph
			...
			if ((m[trackBack].flags&(WordMatch::flagHailedResolveAudience|WordMatch::flagSpecifiedResolveAudience)) && 
					m[previousPrimaryQuote].quoteForwardLink!=lastOpeningPrimaryQuote && // just in case this is introduced in the same paragraph but in a second quote
					m[trackBack].nextQuote==lastOpeningPrimaryQuote && !(m[previousPrimaryQuote].flags&WordMatch::flagLastContinuousQuote) && // quote immediately before - no intervening paragraph
		...
		int ow;
		if (object>=0 && objects[object].objectClass==BODY_OBJECT_CLASS && objects[object].ownerWhere<0 && m[where].previousCompoundPartObject>=0 &&
				m[m[where].previousCompoundPartObject].getObject()>=0 && (ow=objects[m[m[where].previousCompoundPartObject].getObject()].ownerWhere)>=0 &&
		...
		if (where==70409)
		{
			...
			lplog(LOG_RESOLUTION,L"%06d:beforePreviousSpeakers=%s previousSpeakers=%s resolveForSpeaker=%s role=%s lastOpeningPrimaryQuote=%d fl=%s previousPrimaryQuote=%d m[previousPrimaryQuote].quoteForwardLink=%d whereSubjectsInPreviousUnquotedSection=%d.",
				where,objectString(beforePreviousSpeakers,tmpstr).c_str(),objectString(previousSpeakers,tmpstr2).c_str(),(resolveForSpeaker) ? L"true":L"false",
					roleString(where,sRole).c_str(),lastOpeningPrimaryQuote,(m[lastOpeningPrimaryQuote].flags&WordMatch::flagForwardLinkResolveSpeakers) ? L"true":L"false",
					previousPrimaryQuote,m[previousPrimaryQuote].quoteForwardLink,whereSubjectsInPreviousUnquotedSection);
		}
		*/
		if ((m[where].objectRole&HAIL_ROLE) && previousPrimaryQuote>=0 && m[previousPrimaryQuote].quoteForwardLink==lastOpeningPrimaryQuote && m[previousPrimaryQuote].audienceObjectMatches.size()==1)
		{
			wstring tmpstr;
			if (debugTrace.traceSpeakerResolution)
		...
		if (((resolveForSpeaker && !(m[where].objectRole&(IN_PRIMARY_QUOTE_ROLE|IN_SECONDARY_QUOTE_ROLE))) || 
			  ((m[where].objectRole&HAIL_ROLE) && !(m[where].objectRole&IN_SECONDARY_QUOTE_ROLE) && beforePreviousSpeakers.size()<=1)) &&
			   ...
			   m[previousPrimaryQuote].quoteForwardLink!=lastOpeningPrimaryQuote && // beforePreviousSpeakers is not useful if a forward link because it is set in the first link
				!(m[lastOpeningPrimaryQuote].flags&WordMatch::flagForwardLinkResolveSpeakers) && // must have both this AND the previous test (for HAIL and SPEAKER_ROLE)
					 ...
					 !objects[m[whereSubjectsInPreviousUnquotedSection].getObject()].plural))))
		{
			wstring tmpstr,tmpstr2,sRole;
		  if (debugTrace.traceSpeakerResolution)
				lplog(LOG_RESOLUTION,L"%06d:beforePreviousSpeakers=%s previousSpeakers=%s resolveForSpeaker=%s role=%s lastOpeningPrimaryQuote=%d fl=%s previousPrimaryQuote=%d m[previousPrimaryQuote].quoteForwardLink=%d.",
					where,objectString(beforePreviousSpeakers,tmpstr).c_str(),objectString(previousSpeakers,tmpstr2).c_str(),(resolveForSpeaker) ? L"true":L"false",
						m[where].roleString(sRole).c_str(),lastOpeningPrimaryQuote,(m[lastOpeningPrimaryQuote].flags&WordMatch::flagForwardLinkResolveSpeakers) ? L"true":L"false",
						previousPrimaryQuote,m[previousPrimaryQuote].quoteForwardLink);
  ...
  for (I=0; I<HIGHEST; I++)
  {
				 ...
				 (lsOffsets.empty() && (ca=highest[I]->numMatchedAdjectives)>0 && highest[I]->om.salienceFactor>=(MINIMUM_SALIENCE_WITH_MATCHED_ADJECTIVES+ca*MORE_SALIENCE_PER_ADJECTIVE))))
    {
			...
			if (implicitObject(highest[I]->lastWhere) && object>=0 && !objects[object].plural && objects[object].ownerWhere<-1)
			{
				m[where].objectRole|=UNRESOLVABLE_FROM_IMPLICIT_OBJECT_ROLE;
				wstring tmpstr;
		    if (debugTrace.traceSpeakerResolution)
  ...
  if (im->objectMatches.size()==1 && object>=0 && !isPlural && highestActualsf>=SALIENCE_THRESHOLD && 
		  (objects[object].male ^ objects[object].female) && objects[object].objectClass==PRONOUN_OBJECT_CLASS &&
			...
			objects[im->objectMatches[0].object].ownerWhere>=0 &&
			((lsi=in(m[objects[im->objectMatches[0].object].ownerWhere].getObject()))!=localObjects.end() ||
			 (m[objects[im->objectMatches[0].object].ownerWhere].objectMatches.size()==1 && 
			  (lsi=in(m[objects[im->objectMatches[0].object].ownerWhere].objectMatches[0].object))!=localObjects.end())) &&
	...
	if ((im->objectRole&POV_OBJECT_ROLE) && currentSpeakerGroup<speakerGroups.size() && im->objectMatches.size()==1 && 
		  ...
		  !objects[im->objectMatches[0].object].plural && !(im->flags&WordMatch::flagInQuestion))
	{
...
void Source::adjustSaliencesBySubjectRole(int where,int lastBeginS1)
{ LFS
  ...
  for (int I=lastBeginS1; I<where; I++)
				 ...
				 m[I].objectMatches.size()==1))
    {
      ...
      sf+=500;
			...
			if (objects[o].objectClass==BODY_OBJECT_CLASS && objects[o].ownerWhere==-1 && 
        ...
        itos(L"+SUBJADJ[+",sf,lsi->res,L"]");
			...
			if (!pluralAdjective || !objects[o].plural)
				for (unsigned int J=0; J<m[I].objectMatches.size(); J++)
				{
					...
					if (objects[o].objectClass==BODY_OBJECT_CLASS && objects[o].ownerWhere>=0 && 
							(m[where].word->second.inflectionFlags&(MALE_GENDER|FEMALE_GENDER)) && m[objects[o].ownerWhere].getObject()>=0 &&
							(lsi=objects[m[objects[o].ownerWhere].getObject()].lsiOffset)!=cNULL && lsi->includeInSalience(objectToBeMatchedInQuote,quoteIndependentAge) && lsi->om.salienceFactor>=0)
					{
						lsi->om.salienceFactor+=sf;
...
bool Source::physicallyPresentPosition(int where,bool &physicallyEvaluated)
{ LFS
	int beginEntirePosition=m[where].beginObjectPosition; // if this is an adjectival object 
  if (m[where].flags&WordMatch::flagAdjectivalObject) 
		for (; beginEntirePosition>=0 && m[beginEntirePosition].principalWherePosition<0; beginEntirePosition--);
...
bool Source::accompanyingRolePP(int where)
{ LFS
...
bool Source::physicallyPresentPosition(int where,int beginObjectPosition,bool &physicallyEvaluated,bool ignoreTense)
{ LFS
	if (beginObjectPosition<0) return false;
  vector <WordMatch>::iterator im=m.begin()+beginObjectPosition;
	...
	bool inQuestion=(flags&WordMatch::flagInQuestion)!=0;
	bool inPStatement=(flags&WordMatch::flagInPStatement)!=0;
	bool inInfinitivePhrase=(flags&WordMatch::flagInInfinitivePhrase)!=0 && !(or&SENTENCE_IN_ALT_REL_ROLE);
	...
	bool isAdjective=m[where].principalWhereAdjectivalPosition>=0 || (m[where].word->second.inflectionFlags&(SINGULAR_OWNER|PLURAL_OWNER)) ||
	  (m[where].flags&WordMatch::flagNounOwner);
	// in the answer/appeal/letter/missive
	if (notPhysicallyPresentByMissive(where)) return false;
	bool notPhysicallyPresentObject=(m[where].word->second.flags&tFI::notPhysicalObjectByWN) && 
    ...
    if (or&(PLACE_OBJECT_ROLE|MOVEMENT_PREP_OBJECT_ROLE) && isUnresolvable)
		...
		for (int owner=where+1; owner<at && !interveningObject; owner++)
			interveningObject=m[owner].getObject()>=0 && (objects[m[owner].getObject()].male || objects[m[owner].getObject()].female || (m[at].flags&WordMatch::flagNounOwner));
	...
	*/
	...
	if ((or&PREP_OBJECT_ROLE) && !(or&NO_PP_PREP_ROLE) && im->relPrep>=0 && m[im->relPrep].relVerb>=0)
	{
		...
		if (wo>=0 && m[wo].getObject()>=0 && (lsi=in(m[wo].getObject()))!=localObjects.end() && !lsi->physicallyPresent &&
			  (m[wo].word->second.flags&tFI::notPhysicalObjectByWN))
		{
...
bool Source::unResolvablePosition(int where)
{
	LFS
		if (where < 0)
			return false;
...
// a hospital nurse
bool Source::eliminateExactMatchingLocalObject(int where) 
{ LFS
...
void Source::unMatchObjects(int where,vector <cOM> &objectMatches,bool identifiedAsSpeaker)
{ LFS
...
void Source::unMatchObjects(int where,vector <int> &restrictObjects,bool identifiedAsSpeaker)
{ LFS
  for (vector <cOM>::iterator omi=m[where].objectMatches.begin(); omi!=m[where].objectMatches.end(); )
  {
...
bool Source::intersect(int where,set <int> &speakers,bool &allIn,bool &oneIn)
{ LFS
  if (m[where].objectMatches.size())
...
bool Source::intersect(int where,vector <cOM> &speakers,bool &allIn,bool &oneIn)
{ LFS
  if (m[where].objectMatches.size())
    intersect(m[where].objectMatches,speakers,allIn,oneIn);
  else
...
bool Source::intersect(int where,vector <int> &objs,bool &allIn,bool &oneIn)
{ LFS
  if (m[where].objectMatches.size())
...
bool Source::intersect(vector <cOM> &matches,set <int> &speakers,bool &allIn,bool &oneIn)
{ LFS
  allIn=true;
  oneIn=false;
...
bool Source::intersect(set <int> &speakers,vector <cOM> &matches,bool &allIn,bool &oneIn)
{ LFS
  allIn=true;
  oneIn=false;
	for (set <int>::iterator si=speakers.begin(),siEnd=speakers.end(); si!=siEnd; si++)
...
bool Source::intersect(vector <int> &speakers,vector <cOM> &matches,bool &allIn,bool &oneIn)
{ LFS
  allIn=true;
  oneIn=false;
...
bool Source::intersect(vector <int> &matches,set <int> &speakers,bool &allIn,bool &oneIn)
{ LFS
  allIn=true;
  oneIn=false;
  for (unsigned int I=0; I<matches.size(); I++)
...
// are any or all matches in speakers?
bool Source::intersect(vector <int> &o1,vector <int> &o2,bool &allIn,bool &oneIn)
{ LFS
  allIn=true;
  oneIn=false;
...
bool Source::intersect(set <int> &speakers,vector <int> &matches,bool &allIn,bool &oneIn)
{ LFS
  allIn=true;
  oneIn=false;
	for (set <int>::iterator si=speakers.begin(),siEnd=speakers.end(); si!=siEnd; si++)
...
// are any or all speakers in matches?
bool Source::intersect(set <int> &speakers,set <int> &matches,bool &allIn,bool &oneIn)
{ LFS
  allIn=true;
  oneIn=false;
	for (set <int>::iterator si=speakers.begin(),siEnd=speakers.end(); si!=siEnd; si++)
...
bool Source::intersect(vector <cOM> &m1,vector <cOM> &m2,bool &allIn,bool &oneIn)
{ LFS
  allIn=true;
  oneIn=false;
...
bool Source::intersect(vector <tIWMM> &m1,vector <tIWMM> &m2,bool &allIn,bool &oneIn)
{ LFS
  allIn=true;
  oneIn=false;
  for (unsigned int I=0; I<m1.size(); I++)
...
bool Source::intersect(vector <tIWMM> &m1,wchar_t **a,bool &allIn,bool &oneIn)
{ LFS
  allIn=true;
  oneIn=false;
  for (unsigned int I=0; I<m1.size(); I++)
...
bool Source::intersect(vector <cOM> &m1,vector <int> &m2,bool &allIn,bool &oneIn)
{ LFS
  allIn=true;
  oneIn=false;
  for (unsigned int I=0; I<m1.size(); I++)
...
int Source::preferWordOrder(int wordOrderSensitiveModifier,vector <int> &locations)
{ LFS
...
bool Source::mixedPluralityInSameSentence(int where)
{ LFS
	int object=m[where].getObject();
...
void Source::mixedPluralityUsageSubGroupEnhancement(int where)
{ LFS
	if (currentSpeakerGroup<speakerGroups.size() && speakerGroups[currentSpeakerGroup].groupedSpeakers.size())
	{
		...
		{
			int o=m[I].getObject();
			if (o==OBJECT_UNKNOWN_MALE || o==OBJECT_UNKNOWN_FEMALE || 
				...
				singularPronounSeen++;
			else if (o==OBJECT_UNKNOWN_PLURAL || (o>=0 && objects[o].plural))
				pluralPronounSeen++;
		}
		...
		if (o==OBJECT_UNKNOWN_MALE || o==OBJECT_UNKNOWN_FEMALE || (o>=0 && (objects[o].male || objects[o].female) && !objects[o].plural))
			singularPronounSeen++;
		else if (o==OBJECT_UNKNOWN_PLURAL || (o>=0 && objects[o].plural))
			pluralPronounSeen++;
		if (!((singularPronounSeen==0) ^ (pluralPronounSeen==0)))
...
void Source::processSubjectCataRestriction(int where,int subjectCataRestriction)
{ LFS
...
int Source::speakerBefore(int beginQuote,bool &previousParagraph)
{ LFS
...
void Source::setSameAudience(int whereVerb,int speakerObjectPosition,int &audienceObjectPosition)
{ LFS
...
int Source::scanForSpeaker(int where,bool &definitelySpeaker,bool &crossedSectionBoundary,int &audienceObjectPosition)
{ LFS
  if ((unsigned)where+1>=m.size()) return -1;
  definitelySpeaker=true;
	crossedSectionBoundary=false;
  vector <WordMatch>::iterator im=m.begin()+where+1;
  ...
  if ((element=im->pma.queryPattern(L"_VERBREL1",end))!=-1 || vbi!=-1)
  {
    ...
    for (im=m.begin()+speakerObjectPosition; speakerObjectPosition<end; im++,speakerObjectPosition++)
    {
      ...
      if (thinkSayVerb || objects[m[im->principalWherePosition].getObject()].isAgent(true) || isVoice(im->principalWherePosition))
      {
        ...
        {
          if (im->word->first==L"to" && audienceObjectPosition+1<end && (ao=m[audienceObjectPosition+1].getObject())!=UNKNOWN_OBJECT && 
  ...
  else if ((element=im->pma.queryPattern(L"_VERBPAST",end))!=-1 || (im->queryWinnerForm(verbForm)>=0 && (im->word->second.inflectionFlags&VERB_PAST)==VERB_PAST))
  {
      ...
      isVoice(m[speakerObjectPosition].principalWherePosition)))
    {
      ...
      for (audienceObjectPosition=speakerObjectPosition+1,im++; audienceObjectPosition<end; im++,audienceObjectPosition++)
      {
        if (im->word->first==L"to" && audienceObjectPosition+1<end && (ao=m[m[audienceObjectPosition+1].principalWherePosition].getObject())!=UNKNOWN_OBJECT && 
					  (ao<0 || objects[ao].isAgent(true)))
        {
          audienceObjectPosition++;
          return speakerObjectPosition;
  ...
  definitelySpeaker=false;
	if (im->flags&WordMatch::flagIgnoreAsSpeaker) 
	{
          ...
          if (verbPosition+1<(int)m.size() && m[verbPosition+1].word->first==L"to" && (objectPosition=m[verbPosition+2].principalWherePosition)>=0 &&
								...
								(ao<0 || objects[ao].isAgent(true)))
					{
							  ...
							  objects[ao].matchGender(objects[m[objectPosition].getObject()]))
						{
							...
							m[objectPosition].flags|=WordMatch::flagObjectResolved;
...
void Source::addCataSpeaker(int where,int lastBeginS1,int lastRelativePhrase,int lastQ2,int lastVerb,bool definitelySpeaker)
{ LFS
  int element,speakerObjectPosition=-1,speakerObject;
  // skip past verb
  vector <WordMatch>::iterator im=m.begin()+where+2;
...
void Source::setSpeakerMatchesFromPosition(int where,vector <cOM> &objectsToSet,int fromPosition,wchar_t *fromWhere,unsigned __int64 flag)
{ LFS
  ...
  else if (m[fromPosition].getObject()>=0)
  {
	...
	if (flag&(WordMatch::flagSpecifiedResolveAudience|WordMatch::flagDefiniteResolveSpeakers))
	{
  ...
  if (debugTrace.traceSpeakerResolution)
	{
	  wstring tmpstr,tmpstr2;
		bool audience=(flag&(WordMatch::flagForwardLinkAlternateResolveAudience|WordMatch::flagHailedResolveAudience|WordMatch::flagSpecifiedResolveAudience|WordMatch::flagFromLastDefiniteResolveAudience|WordMatch::flagGenderIsAmbiguousResolveAudience|WordMatch::flagMostLikelyResolveAudience))!=0;
...
void Source::setObjectsFromMatchedAtPosition(int where,vector <cOM> &objectsSet,int matchWhere,bool identifiedAsSpeaker,unsigned __int64 flag)
{ LFS
...
bool Source::removeMatchedFromObjects(int where,vector <cOM> &removeFrom,int matchWhere,bool identifiedAsSpeaker)
{ LFS
  if (where<0) return false;
  wstring tmpstr;
...
6 speaker
*/
void Source::resolveSpeakersUsingPreviousSubject(int where)
{ LFS
  ...
  wstring tmpstr;
	bool oneIn,allIn;
	if (intersect(whereSubjectsInPreviousUnquotedSection,speakerGroups[sg].povSpeakers,allIn,oneIn) && (m[firstUnresolved].flags&WordMatch::flagGenderIsAmbiguousResolveSpeakers) &&
  ...
  if (speakersFound.size()!=1) 
	...
	if (speakersFound.size()==1 && 
			!((m[immediatelyAfterSubject].flags)&WordMatch::flagDefiniteResolveSpeakers) &&
			sgAt<(signed)speakerGroups.size() && (speakerGroups[sgAt].speakers.size())>2 &&
			((m[immediatelyAfterSubject].flags&(WordMatch::flagAlternateResolveBackwardFromDefiniteSpeakers|WordMatch::flagEmbeddedStoryResolveSpeakers)) ||
			unresolvedSpeakers[urs]<0) &&
			(!((m[immediatelyAfterSubject].flags)&WordMatch::flagSpecifiedResolveAudience) || m[m[immediatelyAfterSubject].audiencePosition].getObject()!=speakersFound[0]))
	{
		...
		setObjectsFromMatchedAtPosition(immediatelyAfterSubject,m[immediatelyAfterSubject].objectMatches,objects[speakersFound[0]].originalLocation,false,WordMatch::flagAlternateResolveForwardFromLastSubjectSpeakers);
      ...
      for (set <int>::iterator si=speakerGroups[currentSpeakerGroup].speakers.begin(),siEnd=speakerGroups[currentSpeakerGroup].speakers.end(); si!=siEnd; si++)
        if (in(*si,m[immediatelyAfterSubject].objectMatches)==m[immediatelyAfterSubject].objectMatches.end())
          m[immediatelyAfterSubject].audienceObjectMatches.push_back(cOM(*si,SALIENCE_THRESHOLD));
			m[immediatelyAfterSubject].flags|=WordMatch::flagAudienceFromSpeakerGroupResolveAudience;
		}
	  for (vector <cOM>::iterator nmi=m[immediatelyAfterSubject].objectMatches.begin(),nmiEnd=m[immediatelyAfterSubject].objectMatches.end(); nmi!=nmiEnd; nmi++)
		...
		m[immediatelyAfterSubject].flags|=WordMatch::flagAlternateResolveForwardFromLastSubjectSpeakers;
		if (m[immediatelyAfterSubject].audienceObjectMatches.size()==1 && m[immediatelyAfterSubject].audienceObjectMatches[0].object==speakersFound[0])
		{
			...
			for (set <int>::iterator si=speakerGroups[sgAt].speakers.begin(); si!=speakerGroups[sgAt].speakers.end(); si++)
		      ...
		      for (int forwardLink=m[immediatelyAfterSubject].quoteForwardLink; forwardLink>=0; forwardLink=m[forwardLink].quoteForwardLink)
						objects[*si].speakerLocations.insert(forwardLink);
					if (m[immediatelyAfterSubject].flags&WordMatch::flagEmbeddedStoryResolveSpeakers)
						for (int embeddedStoryLink=m[immediatelyAfterSubject].nextQuote; embeddedStoryLink>=0 && (m[embeddedStoryLink].flags&WordMatch::flagEmbeddedStoryResolveSpeakers) && (m[embeddedStoryLink].speakerPosition<0 || (m[embeddedStoryLink].flags&WordMatch::flagGenderIsAmbiguousResolveSpeakers)); embeddedStoryLink=m[embeddedStoryLink].nextQuote)
						{
				      ...
				      for (int forwardLink=m[embeddedStoryLink].quoteForwardLink; forwardLink>=0; forwardLink=m[forwardLink].quoteForwardLink)
							{
								objects[*si].speakerLocations.insert(forwardLink);
								if (m[embeddedStoryLink].speakerPosition>=0)
									objects[*si].speakerLocations.insert(m[embeddedStoryLink].speakerPosition);
			...
			m[immediatelyAfterSubject].flags|=WordMatch::flagAlternateResolveForwardFromLastSubjectAudience;
		}
    for (int forwardLink=m[immediatelyAfterSubject].quoteForwardLink; forwardLink>=0; forwardLink=m[forwardLink].quoteForwardLink)
    {
			setSpeakerMatchesFromPosition(where,m[forwardLink].objectMatches,immediatelyAfterSubject,L"override using previous subject",m[immediatelyAfterSubject].flags|WordMatch::flagForwardLinkResolveSpeakers);
			m[forwardLink].audienceObjectMatches=m[immediatelyAfterSubject].audienceObjectMatches;
    }
		if (m[immediatelyAfterSubject].flags&WordMatch::flagEmbeddedStoryResolveSpeakers)
			for (int embeddedStoryLink=m[immediatelyAfterSubject].nextQuote; embeddedStoryLink>=0 && (m[embeddedStoryLink].flags&WordMatch::flagEmbeddedStoryResolveSpeakers) && (m[embeddedStoryLink].speakerPosition<0 || (m[embeddedStoryLink].flags&WordMatch::flagGenderIsAmbiguousResolveSpeakers)); embeddedStoryLink=m[embeddedStoryLink].nextQuote)
			{
				setSpeakerMatchesFromPosition(where,m[embeddedStoryLink].objectMatches,immediatelyAfterSubject,L"override using previous subject",m[immediatelyAfterSubject].flags|WordMatch::flagForwardLinkResolveSpeakers);
				if (m[embeddedStoryLink].speakerPosition>=0)
					setSpeakerMatchesFromPosition(where,m[m[embeddedStoryLink].speakerPosition].objectMatches,immediatelyAfterSubject,L"override using previous subject",m[immediatelyAfterSubject].flags|WordMatch::flagForwardLinkResolveSpeakers);
				...
				for (int forwardLink=m[embeddedStoryLink].quoteForwardLink; forwardLink>=0; forwardLink=m[forwardLink].quoteForwardLink)
				{
					...
					if (m[forwardLink].speakerPosition>=0)
						setSpeakerMatchesFromPosition(where,m[m[forwardLink].speakerPosition].objectMatches,immediatelyAfterSubject,L"override using previous subject",m[immediatelyAfterSubject].flags|WordMatch::flagForwardLinkResolveSpeakers);
					m[forwardLink].audienceObjectMatches=m[immediatelyAfterSubject].audienceObjectMatches;
				}
			}
...
void Source::setAudience(int where,int whereQuote,int currentSpeakerWhere)
{ LFS
	wstring tmpstr;
	...
	else if (!(m[whereQuote].flags&(WordMatch::flagSpecifiedResolveAudience|WordMatch::flagForwardLinkAlternateResolveAudience)) ||
			m[whereQuote].audienceObjectMatches.size()!=1)
	{
		setSpeakerMatchesFromPosition(whereQuote,m[whereQuote].audienceObjectMatches,currentSpeakerWhere,L"audience forward link",m[where].flags|WordMatch::flagForwardLinkAlternateResolveAudience);
...
void Source::resolveAudienceByAlternation(int currentSpeakerWhere,bool definitelySpeaker,int urs,bool forwards)
{ LFS
  wstring tmpstr;
	unsigned __int64 flagAlternation=(forwards) ? WordMatch::flagAlternateResolveForwardFromLastSubjectAudience : WordMatch::flagAlternateResolveBackwardFromDefiniteAudience; // reject alternate only if going backward, not forward from last subject
  ...
  if (m[ursWhere].objectMatches.size()!=1 || 
		  (definitelySpeaker && (m[ursWhere].flags&(WordMatch::flagMostLikelyResolveAudience|
				WordMatch::flagDefiniteResolveSpeakers|WordMatch::flagAlternateResolveBackwardFromDefiniteSpeakers|WordMatch::flagAlternateResolveForwardAfterDefinite|
				WordMatch::flagFromLastDefiniteResolveAudience|WordMatch::flagSpecifiedResolveAudience|WordMatch::flagAlternateResolveBackwardFromDefiniteAudience|
				WordMatch::flagFromPreviousHailResolveSpeakers|WordMatch::flagAlternateResolveForwardFromLastSubjectSpeakers))==WordMatch::flagMostLikelyResolveAudience) ||
       ...
       in(m[ursWhere].objectMatches[0].object,m[currentSpeakerWhere].objectMatches)==m[currentSpeakerWhere].objectMatches.end()))
  {
		...
		else if (propagateForward=!(m[ursWhere].flags&(flagAlternation|WordMatch::flagAlternateResolveForwardAfterDefinite|
							WordMatch::flagFromLastDefiniteResolveAudience|WordMatch::flagSpecifiedResolveAudience|
							WordMatch::flagFromPreviousHailResolveSpeakers)) ||
      ...
      lplog(LOG_RESOLUTION,L"%06d:set audience matches rejected because alternate already set",ursWhere);
		if (!audienceIsSpeaker && (!(m[ursWhere].flags&(WordMatch::flagAlternateResolveBackwardFromDefiniteSpeakers|WordMatch::flagAlternateResolveForwardFromLastSubjectSpeakers)) || (m[ursWhere].objectMatches.size()!=1)))
		{
      ...
      lplog(LOG_RESOLUTION,L"%06d:remove from matches rejected because alternate already set",ursWhere);
		...
		if (propagateForward)
		{
			for (int forwardLink=m[ursWhere].quoteForwardLink; forwardLink>=0; forwardLink=m[forwardLink].quoteForwardLink)
				setAudience(ursWhere,forwardLink,currentSpeakerWhere);
			if (m[ursWhere].flags&WordMatch::flagEmbeddedStoryResolveSpeakers)
			{
				unsigned __int64 embeddedStoryType=m[ursWhere].flags&(WordMatch::flagSecondEmbeddedStory|WordMatch::flagFirstEmbeddedStory);
				for (int embeddedStoryLink=m[ursWhere].nextQuote; embeddedStoryLink>=0 && 
					(m[embeddedStoryLink].flags&WordMatch::flagEmbeddedStoryResolveSpeakers) && 
					embeddedStoryType==(m[embeddedStoryLink].flags&(WordMatch::flagSecondEmbeddedStory|WordMatch::flagFirstEmbeddedStory)) &&
					(m[embeddedStoryLink].speakerPosition<0 || (m[embeddedStoryLink].flags&WordMatch::flagGenderIsAmbiguousResolveSpeakers)); embeddedStoryLink=m[embeddedStoryLink].nextQuote)
				{
  ...
  if (m[ursWhere].objectMatches.size()<=1 && definitelySpeaker && 
		  (currentSpeakerGroup>=speakerGroups.size() || speakerGroups[currentSpeakerGroup].speakers.size()<=2))
		m[ursWhere].flags|=WordMatch::flagAlternateResolutionFinishedSpeakers;
}

bool Source::matchedObjectsEqual(vector <cOM> &match1,vector <cOM> &match2)
{ LFS
...
bool Source::resolveMatchesByAlternation(int currentSpeakerWhere,bool definitelySpeaker,int urs,bool forwards)
{ LFS
  wstring tmpstr;
  int ursWhere=abs(unresolvedSpeakers[urs]),o;
	bool notSpeakersResolved=false;
	if (!(m[ursWhere].flags&(WordMatch::flagAlternateResolveBackwardFromDefiniteSpeakers|WordMatch::flagAlternateResolveForwardAfterDefinite|WordMatch::flagAlternateResolveForwardFromLastSubjectSpeakers)) || (m[ursWhere].objectMatches.size()!=1))
	{
		...
		{
			// if marked WordMatch::flagMostLikelyResolveSpeakers, and speakerGroup > 1 speaker with matching gender, allow this, hoping that the reresolve would solve it.
			bool allIn,oneIn;
			int genderAgreement=0;
			if ((m[ursWhere].flags&WordMatch::flagMostLikelyResolveSpeakers) && speakerGroups[currentSpeakerGroup].speakers.size()>2)
				for (set<int>::iterator si=speakerGroups[currentSpeakerGroup].speakers.begin(),siEnd=speakerGroups[currentSpeakerGroup].speakers.end(); si!=siEnd; si++)
					genderAgreement+=(objects[*si].matchGender(objects[m[ursWhere].audienceObjectMatches[0].object])) ? 1 : 0;
			if (genderAgreement<=1)
			{
				bool fromLikely=(m[ursWhere].flags&(WordMatch::flagMostLikelyResolveSpeakers|WordMatch::flagMostLikelyResolveAudience))==(WordMatch::flagMostLikelyResolveSpeakers|WordMatch::flagMostLikelyResolveAudience);
				// only pure most likely, going forwards and the previous connected quote does not have the same speaker
				bool overrideLikely=fromLikely && m[ursWhere].previousQuote>=0 && forwards && !(m[m[ursWhere].previousQuote].flags&WordMatch::flagLastContinuousQuote) && 
		...
		if (!(m[ursWhere].flags&WordMatch::flagFromPreviousHailResolveSpeakers) ||
				  ...
				  (m[currentSpeakerWhere].objectMatches.size()==1 && in(m[currentSpeakerWhere].objectMatches[0].object,m[ursWhere].objectMatches)!=m[ursWhere].objectMatches.end()))))
		{
			if (m[ursWhere].speakerPosition<0 || m[m[ursWhere].speakerPosition].getObject()<0 || 
				...
				setObjectsFromMatchedAtPosition(ursWhere,m[ursWhere].objectMatches,currentSpeakerWhere,false,(forwards) ? WordMatch::flagAlternateResolveForwardFromLastSubjectSpeakers : WordMatch::flagAlternateResolveBackwardFromDefiniteSpeakers);
			else
			{
				if (debugTrace.traceSpeakerResolution)
		...
		if (definitelySpeaker)
			m[ursWhere].flags|=(forwards) ? WordMatch::flagAlternateResolveForwardFromLastSubjectSpeakers : WordMatch::flagAlternateResolveBackwardFromDefiniteSpeakers;
		notSpeakersResolved=reEvaluateHailedObjects(ursWhere,true);
		for (int forwardLink=m[ursWhere].quoteForwardLink; forwardLink>=0; forwardLink=m[forwardLink].quoteForwardLink)
			setSpeakerMatchesFromPosition(forwardLink,m[forwardLink].objectMatches,currentSpeakerWhere,L"resolve speakers forward Link",m[ursWhere].flags|WordMatch::flagForwardLinkAlternateResolveSpeakers);
  ...
  // reject this resolution if removing the speaker from counterspeakers would result in no counterspeakers at all
	if (m[ursWhere].audienceObjectMatches.size()!=1 ||
      (m[ursWhere].audienceObjectMatches[0].object!=m[currentSpeakerWhere].getObject() &&
       in(m[ursWhere].audienceObjectMatches[0].object,m[currentSpeakerWhere].objectMatches)==m[currentSpeakerWhere].objectMatches.end()))
	{
		if (!(m[ursWhere].flags&WordMatch::flagAlternateResolveBackwardFromDefiniteAudience) || (m[ursWhere].audienceObjectMatches.size()!=1))
...
struct sFlagString {
	...
	{ WordMatch::flagAlternateResolveForwardAfterDefinite, L"AlternateForwardAfterDefinite " },

	{ WordMatch::flagFromPreviousHailResolveSpeakers, L"FromPreviousHail " },
	{ WordMatch::flagMostLikelyResolveSpeakers, L"MostLikely " },
	{ WordMatch::flagGenderIsAmbiguousResolveSpeakers, L"GenderAmbiguous " },
	{ WordMatch::flagForwardLinkAlternateResolveSpeakers, L"ForwardLinkAlternate " },
	{ WordMatch::flagForwardLinkResolveSpeakers, L"ForwardLink " },
	{ WordMatch::flagDefiniteResolveSpeakers, L"Definite " },
	{ WordMatch::flagAlternateResolveForwardFromLastSubjectSpeakers, L"AlternateForwardFromLastSubject " },
	{ WordMatch::flagAlternateResolveBackwardFromDefiniteSpeakers, L"AlternateBackwardFromDefinite " },
	{ WordMatch::flagAlternateResolutionFinishedSpeakers, L"AlternateFinished " },
	{ WordMatch::flagEmbeddedStoryResolveSpeakers, L"EmbeddedStory " },
	{ 0,NULL },
	{ WordMatch::flagMostLikelyResolveAudience, L"MostLikely " },
	{ WordMatch::flagGenderIsAmbiguousResolveAudience, L"GenderAmbiguous " },
	{ WordMatch::flagForwardLinkResolveAudience, L"ForwardLink " },
	{ WordMatch::flagForwardLinkAlternateResolveAudience, L"ForwardLinkAlternate " },
	{ WordMatch::flagFromLastDefiniteResolveAudience, L"FromLastDefinite " },
	{ WordMatch::flagSpecifiedResolveAudience, L"Audience " },
	{ WordMatch::flagHailedResolveAudience, L"Hailed " },
	{ WordMatch::flagAlternateResolveForwardFromLastSubjectAudience, L"AlternateForwardFromLastSubject " },
	{ WordMatch::flagAlternateResolveBackwardFromDefiniteAudience, L"AlternateBackwardFromDefinite " },
	{ WordMatch::flagAudienceFromSpeakerGroupResolveAudience, L"AudienceFromSpeakerGroup " },
	{ 0,NULL }
};

wstring Source::speakerResolutionFlagsString(__int64 flags,wstring &tmpstr)
{ LFS
	tmpstr.clear();
	unsigned int I;
...
void Source::printUnresolvedLocation(int urs)
{ LFS
	int ursWhere=abs(unresolvedSpeakers[urs]);
	bool finished=(m[ursWhere].flags&WordMatch::flagAlternateResolutionFinishedSpeakers)!=0,notMatched=(unresolvedSpeakers[urs]<0); // objectsNotMatched is unresolved;
	wstring tmpstr,tmpstr2,tmpstr3;
...
void Source::resolveSpeakersByAlternationForwards(int where,int currentSpeakerWhere,bool definitelySpeaker)
{ LFS
  wstring tmpstr,tmpstr2;
  if (debugTrace.traceSpeakerResolution)
  {
  ...
  for (int urs=0; urs<(signed)unresolvedSpeakers.size(); urs++)
  {
		...
		if (!(m[ursWhere].flags&WordMatch::flagAlternateResolutionFinishedSpeakers))
		{
			if ((m[ursWhere].flags&(WordMatch::flagDefiniteResolveSpeakers|WordMatch::flagAudienceFromSpeakerGroupResolveAudience|WordMatch::flagEmbeddedStoryResolveSpeakers)) &&
				  m[ursWhere].objectMatches.size()<=1)
			{
			  if (debugTrace.traceSpeakerResolution)
			...
			if (m[ursWhere].nextQuote>=0 && (m[m[ursWhere].nextQuote].flags&WordMatch::flagDefiniteResolveSpeakers) &&
		...
		urs++;
		if (urs<(signed)unresolvedSpeakers.size() && !(m[ursWhere=abs(unresolvedSpeakers[urs])].flags&WordMatch::flagAlternateResolutionFinishedSpeakers))
		{
			if (counterSpeakerEstablishedPosition>=0)
			{
			...
			{
				if ((m[ursWhere].flags&(WordMatch::flagMostLikelyResolveAudience|WordMatch::flagMostLikelyResolveSpeakers))==(WordMatch::flagMostLikelyResolveAudience|WordMatch::flagMostLikelyResolveSpeakers))
				{
					vector <cOM> om=m[ursWhere].objectMatches;
					m[ursWhere].objectMatches=m[ursWhere].audienceObjectMatches;
					m[ursWhere].audienceObjectMatches=om;
			...
			if (counterSpeakerEstablished>=0 && counterSpeakerEstablishedPosition<0)
			{
				...
				if (m[ursWhere].objectMatches.size()!=1)
				{
					...
					m[ursWhere].flags|=WordMatch::flagAlternateResolveForwardFromLastSubjectSpeakers;
...
void Source::resolvePreviousSpeakersByAlternationBackwards(int where,int currentSpeakerWhere,bool definitelySpeaker)
{ LFS
  wstring tmpstr,tmpstr2;
  if (debugTrace.traceSpeakerResolution)
  {
	...
	__int64 inEmbeddedStory=m[where].flags&(WordMatch::flagEmbeddedStoryResolveSpeakers|WordMatch::flagSecondEmbeddedStory|WordMatch::flagFirstEmbeddedStory);
  for (int urs=unresolvedSpeakers.size()-1; urs>=0; )
  {
		for (; urs>=0 && inEmbeddedStory && !(m[abs(unresolvedSpeakers[urs])].flags&WordMatch::flagAlternateResolutionFinishedSpeakers) && 
					 (m[abs(unresolvedSpeakers[urs])].flags&(WordMatch::flagEmbeddedStoryResolveSpeakers|WordMatch::flagSecondEmbeddedStory|WordMatch::flagFirstEmbeddedStory))==inEmbeddedStory; urs--)
    ...
    if (!(m[abs(unresolvedSpeakers[urs])].flags&WordMatch::flagAlternateResolutionFinishedSpeakers))
			resolveAudienceByAlternation(currentSpeakerWhere,definitelySpeaker,urs,false);
		inEmbeddedStory=m[abs(unresolvedSpeakers[urs])].flags&(WordMatch::flagEmbeddedStoryResolveSpeakers|WordMatch::flagSecondEmbeddedStory|WordMatch::flagFirstEmbeddedStory);
		urs--;
		if (urs<0) break;
		for (; urs>=0 && inEmbeddedStory && !(m[abs(unresolvedSpeakers[urs])].flags&WordMatch::flagAlternateResolutionFinishedSpeakers) && 
					 (m[abs(unresolvedSpeakers[urs])].flags&(WordMatch::flagEmbeddedStoryResolveSpeakers|WordMatch::flagSecondEmbeddedStory|WordMatch::flagFirstEmbeddedStory))==inEmbeddedStory; urs--)
			resolveAudienceByAlternation(currentSpeakerWhere,definitelySpeaker,urs,false);
		if (urs<0) break;
    if (unresolvedSpeakers[urs]>=0 && !(m[abs(unresolvedSpeakers[urs])].flags&WordMatch::flagAlternateResolutionFinishedSpeakers))
			resolveMatchesByAlternation(currentSpeakerWhere,definitelySpeaker,urs,false);
		inEmbeddedStory=m[abs(unresolvedSpeakers[urs])].flags&(WordMatch::flagEmbeddedStoryResolveSpeakers|WordMatch::flagSecondEmbeddedStory|WordMatch::flagFirstEmbeddedStory);
		urs--;
  }
...
bool Source::quotationsImmediatelyBefore(int beginQuote)
{ LFS
...
void Source::displayQuoteContext(unsigned int begin, unsigned int end)
{
	LFS
	wstring qc = L"   ";
...
bool Source::reEvaluateHailedObjects(int beginQuote,bool setMatched)
{ LFS
    ...
    if ((o=m[I].getObject())>=0 && (m[I].objectRole&HAIL_ROLE) &&
				 ...
				 objects[o].objectClass==META_GROUP_OBJECT_CLASS))
		{
			...
			else if (cSG>=0 && cSG<(signed)speakerGroups.size())
			{
				...
				{
					setSpeakerMatchesFromPosition(beginQuote,m[beginQuote].audienceObjectMatches,I,L"Unique reresolve of gender HAIL object audience",WordMatch::flagHailedResolveAudience);
		...
		if (o>=0 && objects[o].objectClass==META_GROUP_OBJECT_CLASS && 
		    objects[o].ownerWhere>=0 && (m[objects[o].ownerWhere].word->second.inflectionFlags&(FIRST_PERSON|SECOND_PERSON))!=0 &&
...
void Source::boostRecentSpeakers(int where,int beginQuote,int speakersConsidered,int speakersMentionedInLastParagraph,bool getMostLikelyAudience)
{ LFS
	vector <cLocalFocus>::iterator lsi;
		  ...
		  m[previousPrimaryQuote].objectMatches.size()==1 && m[previousPrimaryQuote].audienceObjectMatches.size()==1)
	{
		...
		//}
		m[where].flags|=WordMatch::flagResolvedByRecent;
	}
	if (speakerGroups[currentSpeakerGroup].speakers.size()>2 && speakersConsidered>1 && 
		  previousPrimaryQuote>=0 && m[previousPrimaryQuote].nextQuote==beginQuote && !(m[previousPrimaryQuote].flags&WordMatch::flagLastContinuousQuote) &&
		  m[previousPrimaryQuote].objectMatches.size()==1 && m[previousPrimaryQuote].audienceObjectMatches.size()==1)
	{
		if (getMostLikelyAudience)
		{
	...
	if (speakerGroups[currentSpeakerGroup].speakers.size()>2 && speakersConsidered>2 && previousPrimaryQuote>=0 && m[previousPrimaryQuote].nextQuote==beginQuote &&
		  m[previousPrimaryQuote].objectMatches.size()==2 && (m[previousPrimaryQuote].flags&WordMatch::flagResolvedByRecent))
	{
		lsi=in(m[previousPrimaryQuote].objectMatches[0].object);
		if (lsi!=localObjects.end() && lsi->om.salienceFactor>=500)
		{
...
void Source::getMostLikelySpeakers(unsigned int beginQuote,unsigned int endQuote,int lastDefiniteSpeaker,
...
{ LFS
      ...
      lsi->om.salienceFactor+=10000;
		...
		if (lsi->om.salienceFactor>=500 && speakersConsidered>=0)
		{
			if ((previousPrimaryQuote>=0 && lsi->lastWhere>m[previousPrimaryQuote].endQuote) && 
				  (m[previousPrimaryQuote].nextQuote!=beginQuote || (m[previousPrimaryQuote].flags&WordMatch::flagLastContinuousQuote)))
  ...
  if ((localObjects.size() && localObjects[0].om.salienceFactor>=0) || lastDefiniteSpeaker!=-1)
  {
    ...
    if (debugTrace.traceSpeakerResolution)
      lplog(LOG_RESOLUTION,L"%06d:Changing beforePreviousSpeakers from %s to %s",beginQuote,
		...
		if (previousPrimaryQuote>=0 && !(m[previousPrimaryQuote].flags&WordMatch::flagLastContinuousQuote) && 
			  (m[previousPrimaryQuote].flags&(WordMatch::flagSpecifiedResolveAudience|WordMatch::flagHailedResolveAudience)) && 
    ...
    beforePreviousSpeakers=previousSpeakers;
		for (unsigned int I=0; I<beforePreviousSpeakers.size(); I++)
			if (objects[beforePreviousSpeakers[I]].ownerWhere>=0 && m[objects[beforePreviousSpeakers[I]].ownerWhere].getObject()>=0 &&
				  find(beforePreviousSpeakers.begin(),beforePreviousSpeakers.end(),m[objects[beforePreviousSpeakers[I]].ownerWhere].getObject())==beforePreviousSpeakers.end())
				beforePreviousSpeakers.push_back(m[objects[beforePreviousSpeakers[I]].ownerWhere].getObject());
  ...
  for (unsigned int I=beginQuote; I<endQuote; I++)
    ...
    {
       m[I].flags&=~WordMatch::flagObjectResolved;
...
int Source::scanForSpeakers(int begin,int end,int lastBeginS1,int lastRelativePhrase,int lastQ2,int lastVerb,unsigned __int64 roleFlag)
{ LFS
  unsigned int s;
      ...
      if ((!objects[o].male && !objects[o].female && objects[o].neuter) || objects[o].plural)
        ...
        continue;
			...
			if (currentSpeakerGroup<speakerGroups.size() && speakerGroups[currentSpeakerGroup].speakers.find(o)==speakerGroups[currentSpeakerGroup].speakers.end() &&
				  (objects[o].objectClass!=NAME_OBJECT_CLASS || !objects[o].name.justHonorific() || objects[o].name.hon->second.query(L"pinr")<0) &&
					(objects[o].ownerWhere<0 || !(m[objects[o].ownerWhere].word->second.inflectionFlags&FIRST_PERSON))) // my friend
			{
...
bool Source::genderedBodyPartSubstitution(int where,int &speakerObject)
{ LFS
  ...
  // objects with -2 are possessive determiners (his, her, its)
    else if (m[genderedObjectAt].getObject()<UNKNOWN_OBJECT &&
  ...
  if (lastGenderedObjectAt>=0)
  {
      ...
      objects[substituteObject].objectClass==META_GROUP_OBJECT_CLASS))
    {
      ...
      if (objects[substituteObject].objectClass==NAME_OBJECT_CLASS)
        m[at].flags|=WordMatch::flagObjectResolved;
...
bool Source::replaceObjectInSection(int where,int replacementObject,int objectToBeReplaced,wchar_t *fromWhere)
{ LFS
...
int Source::getBodyObject(int o)
{ LFS
  if (objects[o].objectClass==BODY_OBJECT_CLASS && objects[o].ownerWhere>=0)
  {
      if (m[objects[o].ownerWhere].objectMatches.size()==1)
        return m[objects[o].ownerWhere].objectMatches[0].object;
      else if (m[objects[o].ownerWhere].objectMatches.empty())
        return m[objects[o].ownerWhere].getObject();
	}
	return -1;
}

unsigned int Source::numMatchingGenderInSpeakerGroup(int o)
{ LFS
	unsigned int nmg=0;
...
void Source::imposeSpeaker(int beginQuote,int endQuote,int &lastDefiniteSpeaker,int speakerObjectAt,bool definitelySpeaker,
...
{ LFS
  ...
  if (m[speakerObjectAt].principalWherePosition>=0)
  {
		...
		if (m[speakerObjectAt].getObject()>=0 && objects[m[speakerObjectAt].getObject()].ownerWhere>=0)
		  resolveObject(objects[m[speakerObjectAt].getObject()].ownerWhere,definitelySpeaker,false,false,lastBeginS1,lastRelativePhrase,lastQ2,lastVerb,true,false,false);
    resolveObject(speakerObjectAt,definitelySpeaker,false,false,lastBeginS1,lastRelativePhrase,lastQ2,lastVerb,true,false,false);
  }
	// remove observers
  ...
  if (beginQuote>=0)
  {
		...
		if (definitelySpeaker)
		{
			...
			flag=WordMatch::flagDefiniteResolveSpeakers;
			...
			if (o>=0 && (objects[o].objectClass==PRONOUN_OBJECT_CLASS || 
				...
				flag=WordMatch::flagGenderIsAmbiguousResolveSpeakers|WordMatch::flagMostLikelyResolveSpeakers;
		}
  ...
  if (beginQuote>=0)
  {
    ...
    if (audienceObjectPosition>=0)
		{
			__int64 flag=WordMatch::flagSpecifiedResolveAudience;
			int o=m[audienceObjectPosition].getObject();
			if ((m[audienceObjectPosition].objectRole&HAIL_ROLE) && m[audienceObjectPosition].objectMatches.size()>1 && (objects[o].male ^ objects[o].female) &&
				  numMatchingGenderInSpeakerGroup(o)>m[audienceObjectPosition].objectMatches.size())
				flag=WordMatch::flagGenderIsAmbiguousResolveAudience|WordMatch::flagMostLikelyResolveAudience;
      setSpeakerMatchesFromPosition(beginQuote,m[beginQuote].audienceObjectMatches,audienceObjectPosition,howDerived=L"audiencePosition (2)",flag);
    ...
    else if (lastDefiniteSpeaker>=0)
		{
			...
			{
				setSpeakerMatchesFromPosition(beginQuote,m[beginQuote].audienceObjectMatches,lastDefiniteSpeaker,howDerived=L"lastDefiniteSpeaker",WordMatch::flagFromLastDefiniteResolveAudience);
      ...
      if (previousSpeakers.empty())
      {
        ...
        if (unresolvedSpeakers.size()>0)
          unresolvedSpeakers[unresolvedSpeakers.size()-1]=-beginQuote;
				m[beginQuote].flags|=WordMatch::flagMostLikelyResolveAudience;
			}
			else if (previousSpeakers.size()!=1)  
				m[beginQuote].flags|=WordMatch::flagMostLikelyResolveAudience;
			else
				m[beginQuote].flags|=WordMatch::flagAlternateResolveBackwardFromDefiniteAudience; // will block other alternate speakers
...
void Source::addSpeakerObjects(int position,bool toMatched,int where,vector <int> speakers,__int64 resolutionFlag)
{ LFS
	bool pushed=false;
...
void Source::imposeMostLikelySpeakers(unsigned int beginQuote,int &lastDefiniteSpeaker,int audienceObjectPosition)
{ LFS
  ...
  if (previousSpeakersEmpty=previousSpeakers.empty())
	...
	if (!previousSpeakersEmpty && previousPrimaryQuote!=-1 && (m[previousPrimaryQuote].flags&WordMatch::flagSpecifiedResolveAudience))
	{
		...
		{
			addResolveFlag=WordMatch::flagFromPreviousHailResolveSpeakers;
  ...
  // set objectMatches at beginQuote to previousSpeakers
	if (setFlagAlternateResolveForwardAfterDefinite) addResolveFlag|=WordMatch::flagAlternateResolveForwardAfterDefinite;
	addSpeakerObjects(beginQuote,true,beginQuote,previousSpeakers,WordMatch::flagMostLikelyResolveSpeakers|addResolveFlag);
  for (unsigned s=0; s<previousSpeakers.size(); s++)
  {
  ...
  if (audienceObjectPosition>=0 && (currentSpeakerGroup>=speakerGroups.size() || m[audienceObjectPosition].objectMatches.size()<speakerGroups[currentSpeakerGroup].speakers.size()))
	{
    setSpeakerMatchesFromPosition(beginQuote,m[beginQuote].audienceObjectMatches,audienceObjectPosition,L"audienceObjectPosition",WordMatch::flagSpecifiedResolveAudience);
		...
		if (previousPrimaryQuote!=(unsigned int)-1 && m[previousPrimaryQuote].nextQuote==beginQuote && 
			  !(m[previousPrimaryQuote].flags&(WordMatch::flagFromPreviousHailResolveSpeakers|WordMatch::flagDefiniteResolveSpeakers)) &&
    ...
    setSpeakerMatchesFromPosition(beginQuote,m[beginQuote].audienceObjectMatches,lastDefiniteSpeaker,L"definite previous speaker",
			(m[lastDefiniteSpeaker].flags&WordMatch::flagDefiniteResolveSpeakers) ? WordMatch::flagFromLastDefiniteResolveAudience : (WordMatch::flagGenderIsAmbiguousResolveSpeakers|WordMatch::flagMostLikelyResolveAudience));
  else if (beforePreviousSpeakers.size())
	{
		...
		if ((addResolveFlag&WordMatch::flagAlternateResolveForwardAfterDefinite) && intersect(previousSpeakers,beforePreviousSpeakers,allIn,oneIn) && currentSpeakerGroup<speakerGroups.size())
		{
    ...
    addSpeakerObjects(beginQuote,false,beginQuote,beforePreviousSpeakers,WordMatch::flagMostLikelyResolveAudience);
	}
...
void Source::ageSpeaker(int where,bool inPrimaryQuote,bool inSecondaryQuote,vector <cLocalFocus>::iterator &lfi,int amount)
{ LFS
...
void Source::ageSpeaker(int where,vector <cLocalFocus>::iterator &lfi,int amount)
{ LFS
...
bool Source::followObjectChain(int &o)
{ LFS
  int oo=o;
...
int Source::locationBefore(int object,int where)
{ LFS
  int maxLocation=-1;
...
int Source::atBefore(int object,int where)
{ LFS
  int maxLocation=-1;
  for (vector <cObject::cLocation>::iterator loc=objects[object].locations.begin(),locEnd=objects[object].locations.end();
       loc!=locEnd; loc++)
...
bool Source::anyAcceptableLocations(int where,int object)
{ LFS
  for (vector <cObject::cLocation>::iterator loc=objects[object].locations.begin(),locEnd=objects[object].locations.end();
       loc!=locEnd; loc++)
     if (loc->at<where && !(m[loc->at].flags&(WordMatch::flagInQuestion|WordMatch::flagInPStatement)) && !(m[loc->at].objectRole&RE_OBJECT_ROLE))
				return true;
  return false;
}
...
int Source::locationBeforeExceptFuture(int object,int where)
{ LFS
  int maxLocation=-1;
  for (vector <cObject::cLocation>::iterator loc=objects[object].locations.begin(),locEnd=objects[object].locations.end();
       loc!=locEnd; loc++)
...
int Source::ppLocationBetween(int object,int min,int max)
{ LFS
	bool physicallyEvaluated;
...
int Source::locationBefore(int object,int where,bool plural)
{ LFS
  int maxLocation=-1,o;
  for (vector <cObject::cLocation>::iterator loc=objects[object].locations.begin(),locEnd=objects[object].locations.end();
       loc!=locEnd; loc++)
...
int Source::locationBefore(int object,int where,bool plural,bool notInQuestion)
{ LFS
  int maxLocation=-1,o;
  for (vector <cObject::cLocation>::iterator loc=objects[object].locations.begin(),locEnd=objects[object].locations.end();
       loc!=locEnd; loc++)
	{
		if ((o=m[loc->at].getObject())>=0 && plural!=objects[o].plural) continue;
		if (o<0 && plural!=((m[loc->at].word->second.inflectionFlags&(PLURAL|PLURAL_OWNER))!=0)) continue;
		if (notInQuestion && (m[loc->at].flags&WordMatch::flagInQuestion)) continue;
    if (loc->at<where && loc->at>maxLocation) maxLocation=loc->at;
	}
  return (maxLocation!=-1) ? maxLocation : objects[object].originalLocation;
}
...
void Source::processNextSection(int I,int nextSection)
{ LFS
  wstring tmpstr;
  if (debugTrace.traceSpeakerResolution)
...
void Source::clearNextSection(int I,int nextSection)
{ LFS
  wstring tmpstr;
  if (debugTrace.traceSpeakerResolution)
    lplog(LOG_RESOLUTION,L"%06d:______________________END SECTION: %s___________________________",I,phraseString(sections[nextSection].begin,sections[nextSection].endHeader,tmpstr,true).c_str());
...
bool sectionCompareBySpeakerMatches(Source::cSection lhs, Source::cSection rhs)
{ LFS
...
void Source::printSectionStatistics(void)
{ LFS
...
void Source::mergeFocusResolution(int o,int I,vector <int> &lastSubjects,bool &clearBeforeSet)
{ LFS
...
void Source::preferSubgroupMatch(int where,int objectClass,int inflectionFlags,bool inQuote,int speakerObject,bool restrictToAlreadyMatched)
{ LFS
  vector <WordMatch>::iterator im=m.begin()+where;
	int o;
  if ((o=im->getObject())==UNKNOWN_OBJECT || currentSpeakerGroup==0 ||
      (o>=0 &&
  ...
  bool plural=(inflectionFlags&PLURAL)==PLURAL || (inflectionFlags&PLURAL_OWNER)==PLURAL_OWNER;
	bool male=o==OBJECT_UNKNOWN_MALE || o==OBJECT_UNKNOWN_MALE_OR_FEMALE || (o>=0 && objects[o].male);
	bool female=o==OBJECT_UNKNOWN_FEMALE || o==OBJECT_UNKNOWN_MALE_OR_FEMALE || (o>=0 && objects[o].female);
...
bool Source::matchNewHail(int where,vector <cSpeakerGroup>::iterator sg,int o)
{ LFS
...
// in secondary quotes, inPrimaryQuote=false
void Source::accumulateSubjects(int I,int o,bool inPrimaryQuote,bool inSecondaryQuote,int &whereSubject,bool &erasePreviousSubject,vector <int> &lastSubjects)
{ LFS
		  ...
		  (m[I].objectRole&IN_EMBEDDED_STORY_OBJECT_ROLE) && !isNotPhysicallyPresent && o>=0 && currentEmbeddedSpeakerGroup>=0)
	{
	  vector <cSpeakerGroup>::iterator esg=speakerGroups[currentSpeakerGroup].embeddedSpeakerGroups.begin()+currentEmbeddedSpeakerGroup;
		...
		{
			if (objects[o].ownerWhere>=0)
				for (unsigned int omi=0; omi<m[objects[o].ownerWhere].objectMatches.size(); omi++)
					esg->speakers.insert(m[objects[o].ownerWhere].objectMatches[omi].object);
		}
		else
...
int Source::determineSpeaker(int beginQuote,int endQuote,bool inPrimaryQuote,bool noSpeakerAfterward,bool &definitelySpeaker,int &audienceObjectPosition)
{ LFS
...
bool Source::isAgentObject(int object)
{ LFS
...
bool Source::hasAgentObjectOwner(int where,int &ownerWhere)
{ LFS
	return m[where].getObject()>=0 && 
					 (ownerWhere=objects[m[where].getObject()].ownerWhere)>=0 && 
					 ((m[ownerWhere].getObject()>=0 && isAgentObject(m[ownerWhere].getObject())) || 
					 (m[ownerWhere].getObject()==OBJECT_UNKNOWN_MALE || m[ownerWhere].getObject()==OBJECT_UNKNOWN_FEMALE || m[ownerWhere].getObject()==OBJECT_UNKNOWN_MALE_OR_FEMALE));
}

void Source::checkInfinitivePhraseForLocation(vector <tTagLocation> &tagSet,bool locationTense)
{ LFS
	int nextTag=-1;
...
void Source::preparePrepMap(void)
{ LFS
	...
	for (pmi=prepTypesMap.begin(),pmiEnd=prepTypesMap.end(); pmi!=pmiEnd; pmi++)
		if (movementPrepTypes.find(pmi->second)!=movementPrepTypes.end() && (p=Words.query(pmi->first))!=Words.end())
			p->second.flags|=tFI::prepMoveType;
}

// "P","PREPOBJECT","VERBSUBOBJECT"
int Source::evaluatePrepObjectRelation(vector <tTagLocation> &tagSet,int &pIndex,tIWMM &prepWord,int &object,int &wherePrepObject,tIWMM &objectWord)
{ LFS
	// find preposition
	int tag,nextTag=-1;
...
void Source::accumulateLocation(int where,vector <tTagLocation> &tagSet,int subjectObject,bool locationTense)
{ LFS
	...
	if (prepTag<0) return;
	vector < vector <tTagLocation> > tagSets;
	tTagLocation ts=tagSet[prepTag],nts=ts;
	...
	while (true)
	{
		if (startCollectTagsFromTag(debugTrace.traceRelations,prepTagSet,ts,tagSets,-1,false,L"accumulate location")>0)
			for (unsigned int K=0; K<tagSets.size(); K++)
			{
				...
				subobject=-1;
				if (evaluatePrepObjectRelation(tagSets[K],pIndex,prepWord,subobject,wherePrepObject,subObjectWord)>=0 && (prepWord->second.flags&tFI::prepMoveType)!=0)
					movementPrepType=1;
	...
	vector <WordMatch>::iterator im=m.begin()+objects[subobject].begin;
	// allow England
	if (objects[subobject].objectClass!=NAME_OBJECT_CLASS && 
...
int Source::findObject(tTagLocation &tag,int &objectPosition)
{ LFS
	...
	//vector <WordMatch>::iterator im=m.begin()+tag.sourcePosition,imEnd=m.begin()+tag.sourcePosition+tag.len;
...
void Source::assignMetaQueryAudience(int beginQuote,int previousQuote,int primaryObject,int secondaryObject,int secondaryTag,vector <tTagLocation> &tagSet)
{ LFS
	wstring tmpstr,tmpstr2,tmpstr3;
      ...
      if (debugTrace.traceSpeakerResolution)
	      lplog(LOG_RESOLUTION,L"%06d:beginQuote=%d previousQuote=%d secondaryObject=%s pqo=%s",beginQuote,beginQuote,previousQuote,objectString(secondaryObject,tmpstr,true).c_str(),objectString(m[previousQuote].objectMatches[0].object,tmpstr2,true).c_str());
			if (secondaryObject>=0 && secondaryObject!=m[previousQuote].objectMatches[0].object)
			{
			  ...
			  m[whereSecondaryObject].flags|=WordMatch::flagObjectResolved;
			}
		}
...
bool Source::processMetaSpeakerQueryAnswer(int beginQuote,int previousQuote,int lastQuery)
{ LFS
	vector < vector<tTagLocation> > tagSets;
	...
	int whereAboutObject=-1,aboutObject= UNKNOWN_OBJECT,aboutTag;
	...
	for (unsigned int I=0; I<tagSets.size(); I++)
	{
		...
		if (aboutResponseObject!=UNKNOWN_OBJECT)
		{
		...
		else if (aboutObject!=UNKNOWN_OBJECT)
		{
		...
		if (speakerName && primaryObject>=0)
		{
		  ...
		  objects[primaryObject].speakerLocations.insert(beginQuote);
			m[beginQuote].flags|=WordMatch::flagDefiniteResolveSpeakers;
		}
    if (debugTrace.traceSpeakerResolution)
      lplog(LOG_RESOLUTION,L"%06d:beginQuote=%d previousQuote=%d primaryObject=%s secondaryObject=%s aboutObject=%s",I,beginQuote,previousQuote,objectString(primaryObject,tmpstr,true).c_str(),objectString(secondaryObject,tmpstr2,true).c_str(),objectString(aboutObject,tmpstr2,true).c_str());
		...
		if (secondaryObject>=0)
			m[beginQuote].flags|=WordMatch::flagSpecifiedResolveAudience;
		if (debugTrace.traceSpeakerResolution)
...
bool Source::matchAliases(int where,int object,set <int> &objs)
{ LFS
...
bool Source::matchAliases(int where,int object,vector <cOM> &objs)
{ LFS
...
bool Source::matchAliases(int where,int object,int aliasObject)
{ LFS
...
bool Source::matchAliasLocation(int where,int &objectLocation, int &aliasLocation)
{ LFS
	while (objectLocation<(signed)m.size() && m[objectLocation].getObject()==UNKNOWN_OBJECT)
		objectLocation++;
	while (aliasLocation<(signed)m.size() && m[aliasLocation].getObject()==UNKNOWN_OBJECT)
		aliasLocation++;
...
bool Source::matchAlias(int where,int object, int aliasObject)
{ LFS
...
void Source::resolveQuotedPOVObjects(int lastOpeningQuote,int lastClosingQuote)
{ LFS
...
void Source::setEmbeddedStorySpeaker(int where,int &lastDefiniteSpeaker)
{ LFS
	wstring tmpstr;
	if (debugTrace.traceSpeakerResolution)
	...
	setSpeakerMatchesFromPosition(where,m[where].objectMatches,m[where].embeddedStorySpeakerPosition,L"embeddedStorySpeaker",WordMatch::flagEmbeddedStoryResolveSpeakers);
  ...
  for (set <int>::iterator si=speakerGroups[currentSpeakerGroup].speakers.begin(),siEnd=speakerGroups[currentSpeakerGroup].speakers.end(); si!=siEnd; si++)
	...
	m[where].flags|=WordMatch::flagAudienceFromSpeakerGroupResolveAudience;
	if (debugTrace.traceSpeakerResolution)
		lplog(LOG_RESOLUTION,L"%06d:Set last definite speaker to %d.",where,where);
}

void Source::ageIntoNewSpeakerGroup(int where)
{ LFS
      ...
      lfi=localObjects.erase(lfi); // if NAME, ageSpeaker will never delete this, so if not in speakerGroup, then remove
    ...
    else
		{
			...
			if (lfi->lastWhere>=0)
			{
				...
				if (m[lfi->lastWhere].flags&WordMatch::flagAdjectivalObject) 
					for (; beginEntirePosition>=0 && m[beginEntirePosition].principalWherePosition<0; beginEntirePosition--);
			}
...
{ LFS
  ...
  localObjects.clear();
    ...
    im->flags&=~WordMatch::flagObjectResolved;
	}
}

bool Source::setSecondaryQuoteString(int I,vector <int> &secondaryQuotesResolutions)
{ LFS
	...
	if ((secondarySpeaker>=0 || noTextBeforeOrAfter || sentences>=2) && (!noTextBeforeOrAfter || I-lastOpeningSecondaryQuote!=2))
  {
	...
	{
    m[lastOpeningSecondaryQuote].flags|=WordMatch::flagQuotedString;
		#ifdef LOG_QUOTATIONS
...
bool Source::replaceAliasesAndReplacements(set <int> &so)
{ LFS
	wstring tmpstr,tmpstr2,tmpstr3;
	int recur=0;
...
bool Source::replaceSubsequentMatches(set <int> &so,int sgEnd)
{ LFS
	wstring tmpstr,tmpstr2,tmpstr3;
	int recur=0;
	bool replacementsMade=false;
...
bool Source::eraseAliasesAndReplacementsInSpeakerGroup(vector <cSpeakerGroup>::iterator sg,bool eraseSubsequentMatches)
{ LFS
...
void Source::eraseAliasesAndReplacementsInEmbeddedSpeakerGroups(void)
{ LFS
...
bool Source::detectLetterAsObject(int where,int &lastLetterBegin)
{ LFS
...
bool Source::letterDetectionEnd(int where,int whereLetterTo,int lastLetterBegin)
{ LFS
	int endType=-1;
	// 	END: Yours , “Unresolved[tommy:tommy,carter] TWOPENCE . ” 
	// Sincerely, Tommy 
	...
	// 	END: Yours , “Unresolved[tommy:tommy,carter] TWOPENCE . ” 
	if (whereLetterTo>=0 && m[where].word->first==L"“" && (m[where+1].flags&WordMatch::flagAllCaps) &&
	...
	if (setSpeakers>=0 && (lastLetterBegin>=0 || whereLetterTo>=0 || endType==2))
	{
      ...
      objects[letterFrom].locations.push_back(closingResolution-1);
		...
		bool toAllSpeakers=whereLetterTo>=0 && (m[whereLetterTo].objectMatches.empty() && (m[whereLetterTo].word->second.flags&tFI::genericGenderIgnoreMatch) &&
		...
		if (letterFrom>=0 || letterTo>=0)
			...
			{
				for (int linkQuote=letterQuote; linkQuote>=0; linkQuote=m[linkQuote].quoteForwardLink)
				{
					if (letterFrom>=0)
					{
					...
					m[linkQuote].flags|=WordMatch::flagDefiniteResolveSpeakers|WordMatch::flagSpecifiedResolveAudience;
				}
			}
		return true;
	}
...
void Source::resolveMetaReference(int speakerPosition,int quotePosition,int lastBeginS1,int lastRelativePhrase,int lastQ2,int lastVerb)
{ LFS
	...
	m[whereReferredToObject].flags&=~WordMatch::flagObjectResolved;
...
void Source::resolveSpeakers(vector <int> &secondaryQuotesResolutions)
{ LFS 
	resetObjects();
  ...
  unsigned int quotedObjectCounter=0,agingStructuresSeen=0;
	...
	for (int I = 0; I<m.size() && !exitNow; I++)
	{
		...
		if (m[I].getObject() >= 0 || m[I].getObject() < UNKNOWN_OBJECT)
		...
		if (m[I].word->first == L"“")
		{
			...
			if (!(m[I].flags&WordMatch::flagQuotedString))
			{
		...
		else if (isEOS(I) || (I + 1 < ((signed)m.size()) && m[I + 1].word == Words.sectionWord && m[I].word != Words.sectionWord && m[I].word->first != L"," && (sections.empty() || I >= (int)sections[section].endHeader)))
		{
			...
			{
				detectSpaceRelation(J, I, lastSubjects); // so that space relations use disambiguated objects
			...
			{
				if (debugTrace.traceSpeakerResolution)
					lplog(LOG_RESOLUTION, L"%06d:%02d     aging speakers (%s) EOS", I, section, (inPrimaryQuote) ? L"inQuote" : L"outsideQuote");
				for (vector <cLocalFocus>::iterator lfi = localObjects.begin(); lfi != localObjects.end();)
					ageSpeaker(I, inPrimaryQuote, inSecondaryQuote, lfi, 1);
			}
			else if (debugTrace.traceSpeakerResolution)
				lplog(LOG_RESOLUTION, L"%06d:%02d     NOT aging speakers (%s) EOS - %d agingStructuresSeen", I, section, (inPrimaryQuote) ? L"inQuote" : L"outsideQuote", agingStructuresSeen);
			...
			if (!inPrimaryQuote && !inSecondaryQuote)
			{
				if (debugTrace.traceSpeakerResolution)
					...
					I, objectString(lastSubjects, tmpstr).c_str(), whereSubject, objectString(previousLastSubjects, tmpstr).c_str(), wherePreviousLastSubjects);
				...
				((unsigned)currentEmbeddedSpeakerGroup) < speakerGroups[currentSpeakerGroup].embeddedSpeakerGroups.size())
			{
				...
				for (vector <cLocalFocus>::iterator lsi = localObjects.begin(), lsiEnd = localObjects.end(); lsi != lsiEnd; lsi++)
				{
					if (lsi->getAge(false, objectToBeMatchedInQuote, quoteIndependentAge) >= 10 && speakerGroups[currentSpeakerGroup].embeddedSpeakerGroups[currentEmbeddedSpeakerGroup].speakers.find(lsi->om.object) != speakerGroups[currentSpeakerGroup].embeddedSpeakerGroups[currentEmbeddedSpeakerGroup].speakers.end())
					{
						...
						if (J >= begin && (m[m[J].nextQuote].flags&(WordMatch::flagEmbeddedStoryResolveSpeakers | WordMatch::flagEmbeddedStoryResolveSpeakersGap)))
						{
							int getForwardLinkedEnd = J;
							while (m[getForwardLinkedEnd].quoteForwardLink >= 0) getForwardLinkedEnd = m[getForwardLinkedEnd].quoteForwardLink;
							...
							if (debugTrace.traceSpeakerResolution)
								lplog(LOG_RESOLUTION | LOG_SG, L"%d:Aged out %s. Q[%d %d %d] Created another embeddedSpeakerGroup %s.", I, objectString(lsi->om, tmpstr, true).c_str(), J, m[J].endQuote, m[J].nextQuote, toText(speakerGroups[currentSpeakerGroup].embeddedSpeakerGroups[currentEmbeddedSpeakerGroup], tmpstr2));
						}
					}
				}
			}
			...
			if (I + 3 < (signed)m.size() && m[I + 1].word != Words.sectionWord)
			{
				quotesSeenSinceLastSentence = inPrimaryQuote;
				if (debugTrace.traceSpeakerResolution)
					lplog(LOG_RESOLUTION, L"%06d:(EOS)quotesSeenSinceLastSentence=%s", I, (quotesSeenSinceLastSentence) ? L"true" : L"false");
			}
			else
			{
				quotesSeenSinceLastSentence = (lastClosingPrimaryQuote > lastSectionWord);
				if (debugTrace.traceSpeakerResolution)
					lplog(LOG_RESOLUTION, L"%06d:quotesSeenSinceLastSentence=%s not set (end of paragraph)", I, (quotesSeenSinceLastSentence) ? L"true" : L"false");
			}
		}
		else if (m[I].word == Words.sectionWord)
		{
			...
			if (!quotesSeenSinceLastSentence)
			{
				...
				if (lastOpeningPrimaryQuote >= 0)
					m[lastOpeningPrimaryQuote].flags |= WordMatch::flagLastContinuousQuote;
				lastDefiniteSpeaker = -1;
				if (debugTrace.traceSpeakerResolution)
