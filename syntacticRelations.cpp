...
{ LFS
	...
	if (whereVerb>=0)
	{
		...
		if (whereHObject<0)
		{
			...
			int relTraceVerb=m[whereVerb].relVerb,tmp;
			...
			if (relTraceVerb>=0 && m[relTraceVerb].relSubject>=0 && m[s].previousCompoundPartObject!=m[relTraceVerb].relSubject && m[relTraceVerb].relSubject!=s && 
	...
	{
		if (m[where].word->first==L"to" && m[s].relVerb>=0 && m[m[s].relVerb].relVerb<0)
			m[m[s].relVerb].relVerb=whereVerb; // link relVerb with relVerb of infinitive phrase
		return;
	}
	if (whereHObject<0)
	{
...
{ LFS
	...
	if (searchValid || MSTechnique>=0)
	{
		...
		else if (I>=0 && m[I].relVerb>=0 && (m[m[I].relVerb].quoteForwardLink!=tsSense || m[I].relInternalVerb>=0) &&
				...
				else if (MSTechnique<0) // too inconsistent
				{
					if (!infinitiveObjectOfPrep || (infinitiveObjectOfPrep && (m[I].getObject()<0 || !objects[m[I].getObject()].isAgent(true))))
					{
						...
						if (I==whereDesignatedAsInternalSubject && m[J].getObject()>=0 && !objects[m[J].getObject()].neuter &&
							  m[J].relVerb>=0 && m[I].relVerb>=0 && 
								(((m[whereVerb].verbSense&VT_TENSE_MASK)==(m[m[J].relVerb].verbSense&VT_TENSE_MASK)) || 
								 ((m[whereVerb].verbSense&VT_TENSE_MASK)!=(m[m[I].relVerb].verbSense&VT_TENSE_MASK))))
						{
							int JVerbTense=m[m[J].relVerb].verbSense;
							int IVerbTense=m[m[I].relVerb].verbSense;
						...
						int K=-1,originalI=I;
						if (m[I].relVerb<0 || (m[m[I].relVerb].relVerb<0 && !(m[m[I].relVerb].flags&WordMatch::flagInInfinitivePhrase)))
						{
							if (m[J].relVerb>=0 && m[m[J].relVerb].relVerb>=0) I=J;
							else
							{
								...
								if (K>=0 && m[K].relVerb>=0 && m[m[K].relVerb].relVerb>=0) I=K;
								else return;
							}
						}
						if (debugTrace.traceSpeakerResolution)
...
{ LFS
	...
	for (int prepTag=findOneTag(tagSet,L"PREP",-1); prepTag>=0; prepTag=findOneTag(tagSet,L"PREP",prepTag))
	{
		...
		if (startCollectTagsFromTag(debugTrace.traceRelations,prepTagSet,tagSet[prepTag],tagSets,-1,false)>0)
			for (unsigned int K=0; K<tagSets.size(); K++)
			{
						...
						(poTag>=0 && forcePrepObject(tagSets[K],poTag,subobject,wpo,subObjectWord) &&	(wpo=m[tagSets[K][poTag].sourcePosition].principalWherePosition)>=0))
				{
					if (pTag>=0)
					{
						...
						setRelPrep(wpo,wp,3,PREP_OBJECT_SET);
						...
						if (m[whereLastPrep].relPrep<0 && m[wp].relPrep<0)
							setRelPrep(whereLastPrep,wp,4,PREP_PREP_SET);
						...
						if (wp>0 && wp-1!=whereVerb && m[wp-1].relPrep<0 && queryPattern(wp-1,L"__ALLVERB",maxEnd)>=0)
							setRelPrep(wp-1,wp,5,PREP_VERB_SET);
					}
					if (isNot)
...
{ LFS
	...
	if (iverbTag>=0)
	{
		startCollectTagsFromTag(debugTrace.traceRelations,iverbTagSet,tagSet[iverbTag],tagSets,-1,false);
		parentTagLen=tagSet[iverbTag].len;
	}
	else
	{
		startCollectTags(debugTrace.traceRelations,iverbTagSet,where,m[where].pma[infpElement&~patternFlag].pemaByPatternEnd,tagSets,true,false);
	...
	for (unsigned int K=0; K<tagSets.size(); K++)
	{
		...
		if (whereHObject>=0 && whereHVerb>=0)
		{
			...
			if (whereVerb>=0)
			{
				...
				if (m[whereHVerb].previousCompoundPartObject!=-1 && m[whereHVerb].previousCompoundPartObject<whereVerb)
				{
					...
					m[m[whereHVerb].previousCompoundPartObject].relVerb=-1; // IVERB mistakenly attached to verb of previous parent
				}
		...
		if (whereIVerb>=0 && whereVerb>=0 && whereIVerb!=whereVerb)
		{
			...
			if (m[whereIVerb].previousCompoundPartObject!=-1 && m[whereIVerb].previousCompoundPartObject<whereVerb)
			{
				...
				m[m[whereIVerb].previousCompoundPartObject].relVerb=-1; // IVERB mistakenly attached to verb of previous parent
			}
...
{ LFS
		...
		if (withinInfinitivePhrase && m[where].word->first==L"to" && whereSubjects.size() && queryPattern(whereVerb,L"__NOUNRU",maxEnd)==-1)
		{
				...
				for (int is=0; is<(signed)whereSubjects.size(); is++)
			{
				lplog(LOG_RESOLUTION,L"%06d:parent-less infinitive verb@%d subj@%d [NO OBJ=%s MSUBJ=%s P=%s].",where,whereVerb,whereSubjects[is],
							...
							((m[whereSubjects[is]].relVerb>=0 && m[m[whereSubjects[is]].relVerb].relPrep>=0) || (m[where-1].objectRole&PREP_OBJECT_ROLE)) ? L"true":L"false");
			}
		}
	}
	if (withinInfinitivePhrase && whereSubjects.size() && m[whereSubjects[0]].getRelObject()>=0 && m[whereSubjects[0]].relVerb>=0)
	{
		wstring verb;
		unordered_map <wstring, set <int> >::iterator lvtoCi=getVerbClasses(m[whereSubjects[0]].relVerb,verb);
			...
			for (set <int>::iterator vbi=lvtoCi->second.begin(),vbiEnd=lvtoCi->second.end(); vbi!=vbiEnd; vbi++)
				if (vbNetClasses[*vbi].control)
				{
					...
					lplog(LOG_RESOLUTION,L"%06d:infinitive verb@%d - parent object@%d as infinitive subject overrules parent subject@%d because of parent control@%d.",
					      where,whereVerb,whereParentObject,whereSubjects[0],m[whereSubjects[0]].relVerb);
					subjectWords.clear();
					subjectObjects.clear();
					whereSubjects.clear();
	...
	if (firstFreePrep>=0)
	{
		...
		if (whereHVerb>=0 && debugTrace.traceSpeakerResolution)
		...
		if (nextPrep!=firstFreePrep) 
		{
			...
			if (appChain.length()>100 && debugTrace.traceSpeakerResolution)
			...
			{
				setRelPrep(nextPrep,firstFreePrep,6,PREP_PREP_SET);
	...
	for (int objectTag=findOneTag(tagSet,L"OBJECT",-1); objectTag>=0; objectTag=findOneTag(tagSet,L"OBJECT",objectTag),numObjects++)
	{
		...
		{
			if (m[whereObject].relVerb>=0 && m[whereObject].relInternalVerb<0 && (m[where].objectRole&SENTENCE_IN_REL_ROLE))
				m[whereObject].relInternalVerb=m[whereObject].relVerb;
			m[whereObject].relVerb=whereVerb;
		}
		if (m[whereObject].relPrep<0 && whereObject!=wherePrepInObject)
			setRelPrep(whereObject,wherePrepInObject,7,PREP_OBJECT_SET);
		for (unsigned int mo=0; mo<whereMObjects.size(); mo++)
		  ...
		  (m[whereSubjects[0]-1].pma.queryPattern(L"__C1__S1",L"2")!=-1))
	{
		...
		setRelPrep(whereVerb,wherePrepObject-1,8,PREP_VERB_SET);
	...
	for (unsigned int K=0; K<whereSubjects.size(); K++)
	{
		...
		if (m[whereSubjects[K]].relPrep<0 && whereSubjects[K]!=wherePrepInSubject)
			setRelPrep(whereSubjects[K],wherePrepInSubject,9,PREP_OBJECT_SET);
	...
	if (qTagIndex>=0 && whereVerb>=0)
	{
			...
			wv>whereVerb && wv>whereObject)
		{
			...
			m[tagSet[qTagIndex].sourcePosition].relVerb=wv;
			if (debugTrace.traceRole)
		...
		{ 
			if (m[whereVerb].relVerb>=0)
			{
				m[m[whereVerb].relVerb].setRelObject(tagSet[qTagIndex].sourcePosition);
				m[tagSet[qTagIndex].sourcePosition].relVerb=m[whereVerb].relVerb;
				if (debugTrace.traceRole)
					lplog(LOG_ROLE,L"%d:secondary verb@%d assigned relative object @%d.",where,m[whereVerb].relVerb,tagSet[qTagIndex].sourcePosition);
			}
			else
			{
				m[whereVerb].setRelObject(tagSet[qTagIndex].sourcePosition);
				m[tagSet[qTagIndex].sourcePosition].relVerb=whereVerb;
				if (debugTrace.traceRole)
...
{ LFS
  vector <WordMatch>::iterator im=m.begin()+where;
	...
	for (unsigned PMAElement=0; PMAElement<im->pma.count; PMAElement++,pma++)
	{
		...
		{
			tagSets.clear();
			if (startCollectTags(debugTrace.traceRelations,preferredTagSet,where,pma->pemaByPatternEnd,tagSets,true,false)>0)
			{
				for (unsigned int J=0; J<tagSets.size(); J++)
				{
					...
					bool outsideQuoteTruth,inQuoteTruth;
					idType|=evaluateAdditionalRoleTags(where,tagSets[J],pma->len,firstFreePrep,futureBoundPrepositions,inPrimaryQuote,inSecondaryQuote,outsideQuoteTruth,inQuoteTruth,preferredTagSet==iverbTagSet,false,nextVerbInSeries,sense,whereLastVerb,ambiguousSense,inQuotedString,inSectionHeader,begin,end);
				...
				for (unsigned int J=0; J<tagSets.size(); J++)
				{
					...
					if (where>=1)
					{
						...
						if (whereVerb>=0 && m[whereVerb].queryWinnerForm(verbForm)>=0 && m[whereVerb].relPrep<0)
							setRelPrep(whereVerb,wp,10,PREP_VERB_SET);
					}
					m[wp].setRelObject(wpo=m[wpo].principalWherePosition);
					setRelPrep(wpo,wp,11,PREP_OBJECT_SET);
					if (firstFreePrep>=0)
					{
						...
						setRelPrep(lastInChain,wp,12,PREP_PREP_SET);
					}
					if (firstFreePrep<0)
						firstFreePrep=wp;
		...
		if (patterns[pma->getPattern()]->name==L"__APPNOUN" && patterns[pma->getPattern()]->differentiator==L"2" && pma->pemaByChildPatternEnd>=0 &&
				im->relVerb>=0 && im->getRelObject()<0 && (whereObject=m[where+pema[pma->pemaByChildPatternEnd].begin].principalWherePosition)>=0 && m[whereObject].getObject()>=0 &&
				m[whereObject].relVerb<0)
		{
			im->setRelObject(whereObject);
			m[whereObject].relVerb=im->relVerb;
			m[whereObject].relSubject=where;
			m[im->relVerb].setRelObject(whereObject);
			if (debugTrace.traceRole)
				lplog(LOG_ROLE,L"%06d:Appositive subject@%d - verb@%d - object@%d.",where,where,im->relVerb,whereObject);
		}
	}
	return idType;
}
  ...
  for (int I=0; im!=imend; im++,I++)
    ...
    if (setAdditionalRoleTags(I,firstFreePrep,futureBoundPrepositions,inPrimaryQuote,inSecondaryQuote,nextVerbInSeries,sense,whereLastVerb,ambiguousSense,inQuotedString,inSectionHeader,begin,end,tagSets) && lastBeginS1>=0)
      m[lastBeginS1].objectRole|=ID_SENTENCE_TYPE;
		...
		for (unsigned PMAElement=0; PMAElement<im->pma.count; PMAElement++,pma++)
		{
			...
			tagSets.clear();
			if (startCollectTags(debugTrace.traceVerbObjects,verbObjectsTagSet,I,pma->pemaByPatternEnd,tagSets,true,false)>0)
			{
				for (unsigned int J=0; J<tagSets.size(); J++)
				{
	...
	for (unsigned int p=0; p<futureBoundPrepositions.size(); p++)
	{
		...
		if (m[v+1].getRelObject()>=0)
		{
			...
			if (!setAlready)
			{
				setRelPrep(v+1,m[v].relPrep,14,PREP_PREP_SET);
				setRelPrep(v,v+1,15,PREP_VERB_SET);
				if (debugTrace.traceSpeakerResolution)
