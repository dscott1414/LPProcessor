...
int cSource::evaluateISARelation(int parentSourceWhere, int where, vector <cTagLocation>& tagSet, vector <lpwstring>& lookForSubject)
{
	LFS
		int nextTag = -1, verbTagIndex;
	if ((verbTagIndex = findOneTag(tagSet, u"VERB", -1)) < 0) return -1;
	// I am not your enemy.
	...
	if (m[whereVerb].getMainEntry()->first == u"denote")
		isId = true;
	if (!isId ||
		findTagConstrained(tagSet, u"not", nextTag, tagSet[verbTagIndex]) >= 0 || findTagConstrained(tagSet, u"never", nextTag, tagSet[verbTagIndex]) >= 0)
		return -1;
	nextTag = -1;
	tIWMM subjectWord = wNULL, objectWord = wNULL;
	int subjectTag = findTag(tagSet, u"SUBJECT", nextTag), subjectObject, whereSubjectObject;
	...
	{
		lpwstring w = (m[I].word->second.mainEntry != wNULL) ? m[I].word->second.mainEntry->first : m[I].word->first;
	...
	if (!mainEntryMatch)
	{
		lplog(LOG_WIKIPEDIA, u"%06d:%d:Rejected subject %s because of non matching main word@>=%d.", parentSourceWhere,
			where, objectString(subjectObject, tmpstr, true).c_str(), whereSubjectObject);
		return -1;
	}
	// now identify object
	nextTag = -1;
	int objectTag = findTag(tagSet, u"OBJECT", nextTag), objectObject, whereObjectObject;
	...
	lplog(LOG_WIKIPEDIA, u"%06d:%d:Found subject %s having ISA object %s [subType %s].", parentSourceWhere,
		where, objectString(subjectObject, tmpstr, true).c_str(), objectString(objectObject, tmpstr2, true).c_str(),
		(objects[objectObject].getSubType() >= 0) ? OCSubTypeStrings[objects[objectObject].getSubType()] : u"N/A");
	// Llanelly (Welsh) is the name of both a village and its respective parish
	if (m[whereObjectObject].word->first == u"name" && m[whereObjectObject + 1].word->first == u"of" && m[whereObjectObject + 1].getRelObject() >= 0)
	{
		int element = -1;
		if ((element = m[whereObjectObject].pma.queryTag(MNOUN_TAG)) >= 0)
		{
			...
			while (whereCompoundObject >= 0)
			{
				...
				lplog(LOG_WIKIPEDIA, u"%06d:%d:Found subject %s having ISA multi object %s [subType %s].", parentSourceWhere,
					where, objectString(subjectObject, tmpstr, true).c_str(), objectString(objectObject, tmpstr2, true).c_str(),
					(objects[objectObject].getSubType() >= 0) ? OCSubTypeStrings[objects[objectObject].getSubType()] : u"N/A");
		...
		else
		{
			...
			lplog(LOG_WIKIPEDIA, u"%06d:%d:Found subject %s having ISA named object %s [subType %s].", parentSourceWhere,
				where, objectString(subjectObject, tmpstr, true).c_str(), objectString(objectObject, tmpstr2, true).c_str(),
				(objects[objectObject].getSubType() >= 0) ? OCSubTypeStrings[objects[objectObject].getSubType()] : u"N/A");
		}
	}
...
bool cSource::getISARelations(int parentSourceWhere, int where, vector < vector <cTagLocation> >& tagSets, vector <int>& OCTypes, vector <lpwstring>& lookForSubject)
{
	LFS
	...
	for (unsigned PMAElement = 0; PMAElement < im->pma.count; PMAElement++, pma++)
	{
		...
		if ((((patterns[pma->getPattern()]->includesOnlyDescendantsAllOfTagSet) & ((int64_t)1 << subjectVerbRelationTagSet)) != 0))
		{
			tagSets.clear();
			if (startCollectTags(debugTrace.traceRelations, subjectVerbRelationTagSet, where, pma->pemaByPatternEnd, tagSets, true, false, u"get ISA relation") > 0)
			{
				for (unsigned int J = 0; J < tagSets.size(); J++)
				{
					if (debugTrace.traceRelations)
						printTagSet(LOG_RESOLUTION, u"ART", J, tagSets[J], where, pma->pemaByPatternEnd);
...
// Writes OCTypes to path+".attribs". Returns false if copy or lp_wopen fails.
bool writeAttribs(lpwstring path, vector <int>& OCTypes)
{
	LFS
		// heap-allocated (tmalloc/tfree) rather than a ~1.91 MiB stack array
		char* buffer = (char*)tmalloc(MAX_BUF);
	...
	lpwstring attribsPath = lpwstring(path) + u".attribs";
	if ((fd = lp_wopen(attribsPath.c_str(), O_CREAT | O_RDWR | O_BINARY, _S_IREAD | _S_IWRITE)) < 0)
	{
		lplog(LOG_ERROR, u"ERROR:Cannot create path %s - %S (7).", attribsPath.c_str(), sys_errlist[errno]);
		tfree(MAX_BUF, buffer);
		return false;
	}
	::write(fd, buffer, len);
...
int cSource::identifyISARelationTextAnalysis(cQuestionAnswering& qa, int principalWhere, bool parseOnly)
{
	...
	lpchar_t path[1024];
	vector <lpwstring> lookForSubject, wikipediaLinks;
	...
	if (!readAttribs(path, OCTypes))
	{
		...
		lplog(LOG_WIKIPEDIA, u"%06d:Read attribs for %s...", principalWhere, path);
	lpwstring tmpstr;
	...
	if (!disallowNotAPlace && gendered)
	{
		...
		lplog(LOG_WIKIPEDIA, u"%06d:%s reassigned (wikipedia) to not a place [gendered].", principalWhere, objectString(o, tmpstr, false).c_str());
		return 0;
	}
	for (unsigned int I = 0; I < OCTypes.size(); I++)
			...
			objects[o].getSubType() != COUNTRY && objects[o].getSubType() != OCEAN_SEA && objects[o].getSubType() != US_STATE_TERRITORY_REGION)
		{
			if (OCTypes[I] == NUM_SUBTYPES)
			{
				...
				objects[o].isNotAPlace = true;
				objects[o].resetSubType();
				lplog(LOG_WIKIPEDIA, u"%06d:%s reassigned (wikipedia) to not a place.", principalWhere, objectString(o, tmpstr, false).c_str());
			}
			else if (!(objects[o].male ^ objects[o].female) || OCTypes[I] != UNKNOWN_PLACE_SUBTYPE)
			{
				objects[o].setSubType(OCTypes[I]);
				lplog(LOG_WIKIPEDIA, u"%06d:%s reassigned (wikipedia) to the object subtype %s.", principalWhere,
